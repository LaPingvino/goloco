...
// args.push<uint16>(0); // Needed after a user string id
...
// if (static_cast<uint16>(itemIndex) >= _sortedCompanies.size())
// CompanyList::open();
// else
...
// if (static_cast<int16>(playerCompany->cash.var_04) < 0)
...
// func If(CompanyFlags::decreasedPerformance (playerCompany->challengeFlags) else
...
// // 0x004395DE
// static Ui::CursorId onCursor([[maybe_unused]] Ui::Window& window, WidgetIndex_t widgetIndex, [[maybe_unused]] const WidgetId id, [[maybe_unused]] int16 xPos, [[maybe_unused]] int16 yPos, Ui::CursorId fallback)
// switch (widgetIndex)
// case Widx::company_value:
// case Widx::performanceIndex:
