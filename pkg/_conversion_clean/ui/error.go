...
// static uint16 _linebreakCount;     // 0x009C66B3
...
// uint16 strWidth = Gfx::TextRenderer::getStringWidthNewLined(Gfx::Font::medium_bold, &_errorText[0]);
// strWidth = std::clamp<uint16>(strWidth, kMinWidth, kMaxWidth);
// // How many linebreaks?
// uint16 breakLineCount = 0;
...
// uint16 width = strWidth + 2 * kPadding;
// uint16 height = (_linebreakCount + 1) * 10 + 2 * kPadding;
...
// uint16 frameWidth = width - 1;
// uint16 frameHeight = height - 1;
...
// windowPosition.x = std::clamp<int32>(windowPosition.x, 0, Ui::width() - width - 40);
// windowPosition.y = std::clamp<int32>(windowPosition.y, 22, Ui::height() - height - 40);
...
// int32 pan = (error->width / 2) + error->x;
...
// if (_errorCompetitorId == CompanyId::null)
// uint16 xPos = self.x + self.width / 2;
// uint16 yPos = self.y + kPadding;
