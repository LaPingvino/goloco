...
// Window* findAt(int32 x, int32 y);
// Window* findAt(Ui::Point point);
// Window* findAtAlt(int32 x, int32 y);
...
// Window* openWithFlags(uint32 flags);
...
// Window* open(uint8 shortcutIndex);
// namespace Error
...
// SKIPPED C++ SYNTAX: type CloseCallback = any /* std::function<void()> */ 
// Window* open(string_view text, CloseCallback cbClose);
...
// Window* open(string_view captionString);
...
type Browse_type int
	...
	Load Browse_type = 1
	Save Browse_type = 2
// std::optional<string> open(browse_type type, string_view path, const char* filter, StringId titleId);
...
// namespace TextInput
// func OpenTextInput(w Ui::Window, title StringId, message StringId, value StringId, callingWidget int, valueArgs FormatArgumentsView, 1 uint32 inputSize = StringManager::kUserStringSize -) 
