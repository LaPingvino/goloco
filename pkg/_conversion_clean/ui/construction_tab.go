...
// static uint8 _ghostRemovalTrackObjectId; // 0x00522093
...
// static std::optional<TrackPieceId> getRoadPieceId(uint8 trackPiece, uint8 gradient, uint8 rotation);
// static std::optional<TrackPieceId> getTrackPieceId(uint8 trackPiece, uint8 gradient, uint8 rotation);
// // 0x0049B50C
// func Reset()
...
// uint32 trackAndDirection = 0;
// if (cState.constructionRotation < 4)
// trackAndDirection = 0;
// func If(8 any /* cState.constructionRotation < */ ) else
// trackAndDirection = 26 << 3;
// func If(12 any /* cState.constructionRotation < */ ) else
...
// return;
// World::Pos3 loc(cState.x, cState.y, cState.constructionZ);
// uint32 trackAndDirection = (1 << 2) | (cState.constructionRotation & 0x3);
...
// break;
// // 0x0049DB71
// func GetUnusedPiecesRotation() uint64
...
// func If(4 cState.constructionRotation >=) else
// unusedWidgets |= (1 << widx::left_hand_curve_small) | (1 << widx::left_hand_curve) | (1 << widx::left_hand_curve_large) | (1 << widx::right_hand_curve_large) | (1 << widx::right_hand_curve) | (1 << widx::right_hand_curve_small);
...
// // 0x0049DBEC
// func GetUnusedRoadPieces() uint64
// auto unusedWidgets = 0U;
// auto& cState = getConstructionState();
...
// orphan member: return unusedWidgets;
// // 0x0049DB1F
// func GetUnusedTrackPieces(trackObj TrackObject) uint64
// auto unusedWidgets = 0U;
// auto& cState = getConstructionState();
// if (cState.lastSelectedTrackGradient == 2 || cState.lastSelectedTrackGradient == 6 || cState.lastSelectedTrackGradient == 4 || cState.lastSelectedTrackGradient == 8)
// unusedWidgets |= (1 << widx::left_hand_curve_very_small) | (1 << widx::left_hand_curve) | (1 << widx::left_hand_curve_large) | (1 << widx::right_hand_curve_large) | (1 << widx::right_hand_curve) | (1 << widx::right_hand_curve_very_small);
...
// orphan member: return unusedWidgets;
// // 0x0049DAF3
// func GetUnusedTrackSlopes(trackObj TrackObject) uint64
// auto unusedWidgets = 0U;
...
// orphan member: return unusedWidgets;
// func SetMapSelectedTilesFromPiece(pieces any /* []<TrackData::PreviewTrack> */ , origin World::Pos2, rotation uint8)
...
// const uint16 x = cState.x;
// const uint16 y = cState.y;
// if (!road)
...
// func If(!cState.constructionHover) else
...
// uint8 trackGradient = widx::level;
...
// // 0x004A04F8
// std::optional<TrackPieceId> getTrackPieceId(uint8 trackPiece, uint8 gradient, uint8 rotation)
// if (trackPiece == TrackPiece::null)
// return std::nullopt;
// uint8 id = 0;
// switch (trackPiece)
...
// uint8 trackType = cState.trackType & ~(1 << 7);
// if (isRoadPiece)
...
// args.push<uint16>(bridgeObj->maxHeight);
...
// func If(ImageIds::construction_right_turnaround self.widgets[widx::s_bend_dual_track_left].image == ImageIds::construction_left_turnaround || self.widgets[widx::s_bend_dual_track_left].image ==) else
...
// cState.lastSelectedTrackPiece = TrackPiece::s_bend_to_dual_track;
// func If(ImageIds::construction_left_turnaround self.widgets[widx::s_bend_dual_track_right].image ==) else
// cState.lastSelectedTrackPiece = TrackPiece::turnaround;
// else
// cState.lastSelectedTrackPiece = TrackPiece::s_bend_to_single_track;
// activateSelectedConstructionWidgets();
...
// cState.trackCost = GameCommands::FAILURE;
// activateSelectedConstructionWidgets();
// static std::optional<GameCommands::RoadPlacementArgs> getRoadPlacementArgs(const World::Pos3& pos, const uint8 trackPiece, const uint8 gradient, const uint8 rotation);
// static std::optional<GameCommands::TrackPlacementArgs> getTrackPlacementArgs(const World::Pos3& pos, const uint8 trackPiece, const uint8 gradient, const uint8 rotation);
...
// return { height };
// // 0x00478361
// static std::optional<int16> getExistingRoadAtLoc(int16 x, int16 y)
...
// return { startHeight };
// // 0x004A4011
// static std::optional<std::pair<int16, int16>> getExistingTrackAtLoc(int16 x, int16 y)
...
// Audio::playSound(Audio::SoundId::error, int32(Input::getMouseLocation().x));
// return;
// // 0 if nothing currently selected
// func GetMaxConstructHeightFromExistingSelection() int16
// int16 maxHeight = 0;
...
// maxHeight = std::max<int16>(tileHeight->waterHeight + 16, maxHeight);
// orphan member: return maxHeight;
// static std::optional<std::pair<World::TilePos2, int16>> tryMakeRoadJunctionAtLoc(const int16 x, const int16 y)
...
// return std::nullopt;
// static std::optional<std::pair<World::TilePos2, int16>> tryMakeTrackJunctionAtLoc(const int16 x, const int16 y)
...
// return { std::make_pair(World::toTileSpace(*mapPos), existingHeight) };
// return std::nullopt;
// static std::optional<std::pair<World::TilePos2, int16>> getConstructionPos(const int16 x, const int16 y, const int16 baseHeight = std::numeric_limits<int16>::max())
...
// func GetMinPieceHeight(piece any /* []<TrackData::PreviewTrack> */ ) int16
...
// cState.lastSelectedBridge = bridgeType;
// WindowManager::invalidate(WindowType::construction);
// func PlaceRoadGhost(args) return
// else
// cState.ghostRemovalTrackPos = args.pos;
...
// orphan member: Pos2 constructPos;
// int16 constructHeight = 0;
...
// _isDragging = false;
// func OnToolDrag(self [[maybe_unused]] Window, widgetIndex [[maybe_unused]] WidgetIndex_t, id [[maybe_unused]] WidgetId, x [[maybe_unused]] int16, y [[maybe_unused]] int16)
...
// return;
// cState.placeTrackPieceId = pieceId->id;
// int16 constructHeight = getMaxConstructHeightFromExistingSelection();
...
// constructHeight = std::max<int16>(0, constructHeight);
...
// func If(7 any /* cState.trackType (1 << */ ) else
...
// _isDragging = false;
// // 0x0049D4F5
// static Ui::CursorId cursor([[maybe_unused]] Window& self, WidgetIndex_t widgetIndex, [[maybe_unused]] const WidgetId id, [[maybe_unused]] int16 xPos, [[maybe_unused]] int16 yPos, Ui::CursorId fallback)
...
// orphan member: FormatArguments args{};
// args.push<uint32>(cState.trackCost);
// tr.drawStringCentred(Point(x, y), Colour::black, StringIds::build_cost, args);
// func If(0 cState.roadCost != GameCommands::FAILURE cState.roadCost !=) else
// orphan member: FormatArguments args{};
// args.push<uint32>(cState.roadCost);
// tr.drawStringCentred(Point(x, y), Colour::black, StringIds::build_cost, args);
// // 0x0049D106
