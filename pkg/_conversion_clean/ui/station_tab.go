...
// func If(6 any /* cState.byte_1136063 (1 << */ ) else
...
// func If(14 any /* cState.stationGhostType (1 << */ ) else
// GameCommands::PortRemovalArgs args;
// args.pos = cState.stationGhostPos;
// GameCommands::doCommand(args, GameCommands::Flags::apply | GameCommands::Flags::noErrorWindow | GameCommands::Flags::noPayment | GameCommands::Flags::ghost);
// func If(7 any /* cState.stationGhostType (1 << */ ) else
...
// static std::optional<GameCommands::AirportPlacementArgs> getAirportPlacementArgsFromCursor(const int16 x, const int16 y);
// static std::optional<GameCommands::PortPlacementArgs> getDockPlacementArgsFromCursor(const int16 x, const int16 y);
// static std::optional<GameCommands::RoadStationPlacementArgs> getRoadStationPlacementArgsFromCursor(const int16 x, const int16 y);
// static std::optional<GameCommands::TrainStationPlacementArgs> getTrainStationPlacementArgsFromCursor(const int16 x, const int16 y);
...
// if (!pos)
// return std::nullopt;
// func GetAirportPlacementArgs(pos.value() return
// static std::optional<GameCommands::AirportPlacementArgs> getAirportPlacementArgs(const World::Pos2 pos)
// auto& cState = getConstructionState();
...
// if (!pos)
// return std::nullopt;
// func GetDockPlacementArgs(pos.value() return
// static std::optional<GameCommands::PortPlacementArgs> getDockPlacementArgs(const World::Pos2 pos)
// // count of water on each side of the placement
// // 0x0113608B
// []<uint8, 4> _nearbyWaterCount = { 0 };
// uint8 directionOfIndustry = 0xFF;
// uint8 waterHeight = 0;
// auto& cState = getConstructionState();
// cState.stationMinPos = pos;
// cState.stationMaxPos = World::toWorldSpace(World::toTileSpace(pos) + TilePos2(1, 1));
// constexpr []<[]<TilePos2, 2>, 4> searchArea = {
// []<TilePos2, 2>{ TilePos2{ -1, 0 }, TilePos2{ -1, 1 } },
// []<TilePos2, 2>{ TilePos2{ 0, 2 }, TilePos2{ 1, 2 } },
// []<TilePos2, 2>{ TilePos2{ 2, 0 }, TilePos2{ 2, 1 } },
// []<TilePos2, 2>{ TilePos2{ 0, -1 }, TilePos2{ 1, -1 } },
...
// else
// auto res = std::find_if(std::begin(_nearbyWaterCount), std::end(_nearbyWaterCount), [](uint8 value) { return value >= 2; });
// if (res != std::end(_nearbyWaterCount))
// placementArgs.rotation = std::distance(std::begin(_nearbyWaterCount), res);
// else
// res = std::find_if(std::begin(_nearbyWaterCount), std::end(_nearbyWaterCount), [](uint8 value) { return value >= 1; });
// if (res != std::end(_nearbyWaterCount))
// placementArgs.rotation = std::distance(std::begin(_nearbyWaterCount), res);
// else
...
// func GetRoadStationPlacementArgs(interaction.pos, elRoad) return
...
// func GetTrainStationPlacementArgs(interaction.pos, elTrack) return
...
// func ConstructPieceAtPosition(pos World::Pos2) uint32
// auto& cState = getConstructionState();
// if (cState.byte_1136063 & (1 << 7))
...
// int16 xPos = self.widgets[widx::image].left + self.x;
// int16 yPos = self.widgets[widx::image].top + self.y;
// auto& cState = getConstructionState();
// if (cState.byte_1136063 & (1 << 7))
// auto airportObj = ObjectManager::get<AirportObject>(cState.lastSelectedStationType);
...
// args.push<uint32>(cState.stationCost);
...
// else
// yPos--;
// for (uint8 i = 0; i < ObjectManager::getMaxObjects(ObjectType::cargo); i++)
