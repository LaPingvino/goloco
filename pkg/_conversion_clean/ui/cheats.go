...
// // Vehicles tab
// static constexpr uint32 vehiclesTabImageIds[] = {
...
// return std::max<int32>(0, std::min<int32>(getMonthTotalDay(date.year, date.month) - 1, date.day));
...
// _date.year = std::max<int32>(OpenLoco::Scenario::kMinYear, Math::Bound::sub(_date.year, timeStepSize));
// break;
...
// _date.day = std::max<int32>(0, Math::Bound::sub(_date.day, timeStepSize));
// break;
// case Widx::day_step_increase:
// _date.day = std::min<int32>(getMonthTotalDay(_date.year, _date.month) - 1, Math::Bound::add(_date.day, timeStepSize));
// break;
