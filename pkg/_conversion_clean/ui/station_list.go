...
// if (window != nullptr)
// for (uint16 i = 0; i < window->var_83C; i++)
...
// Window* open(CompanyId companyId, uint8 type)
// if (type > 4)
...
// args.push<uint16>(window.var_83C);
// // 0x0049157F
...
// args.push<uint16>(enumValue(station->town));
// args.push<uint16>(getTransportIconsFromStationFlags(station->flags));
...
// uint32 image = Gfx::recolour(skin->img + tab.imageId, companyColour);
...
// uint32 image = Gfx::recolour(competitor->images[enumValue(company->ownerEmotion)], company->mainColours.primary);
...
// break;
// // 0x00491A0C
// func OnScrollMouseDown(window Ui::Window, x [[maybe_unused]] int16, y int16, scroll_index [[maybe_unused]] uint8)
...
// // 0x004919D1
// func OnScrollMouseOver(window Ui::Window, x [[maybe_unused]] int16, y int16, scroll_index [[maybe_unused]] uint8)
// window.flags &= ~(WindowFlags::notScrollView);
// uint16 currentRow = y / kRowHeight;
// int16 currentStation = -1;
// if (currentRow < window.var_83C)
