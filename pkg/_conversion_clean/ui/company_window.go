...
// const uint32 image = Gfx::recolour(competitor->images[enumValue(company->ownerEmotion)] + 1, company->mainColours.primary);
// const uint16 x = self.x + self.widgets[widx::face].left + 1;
// const uint16 y = self.y + self.widgets[widx::face].top + 1;
// drawingCtx.drawImage(x, y, image);
// // If the owner's been naughty, draw some jail bars over them.
// if (company->jailStatus != 0)
// const uint32 image = ImageIds::owner_jailed;
// const uint16 x = self.x + self.widgets[widx::face].left + 1;
// const uint16 y = self.y + self.widgets[widx::face].top + 1;
// drawingCtx.drawImage(x, y, image);
// // Draw owner name
...
// static uint8 _headquarterConstructionRotation;
...
// auto skin = ObjectManager::get<InterfaceSkinObject>();
// uint32 image = skin->img + InterfaceSkin::ImageIds::build_headquarters;
...
// const auto competitor = ObjectManager::get<CompetitorObject>(company.competitorId);
// orphan member: FormatArguments args{};
// args.push<uint16>(competitor->intelligence);
...
// y += 10;
// orphan member: FormatArguments args{};
// args.push<uint16>(competitor->aggressiveness);
...
// y += 10;
// orphan member: FormatArguments args{};
// args.push<uint16>(competitor->competitiveness);
...
// y += 10;
// static []<StringId, 6> transportTypeCountString = {
...
// func If(CompanyFlags::increasedPerformance (company->challengeFlags) else
...
// static std::optional<GameCommands::HeadquarterPlacementArgs> getHeadquarterPlacementArgsFromCursor(const int16 mouseX, const int16 mouseY)
...
// uint8 flags = GameCommands::Flags::apply | GameCommands::Flags::preventBuildingClearing;
...
// if (Config::get().gridlinesOnLandscape)
// vpFlags |= ViewportFlags::gridlines_on_landscape;
// func If(view self.savedView !=) else
...
// static constexpr Ui::Size kWindowSize = { 265, 252 };
	...
	Check_steam_locomotives Widx = 11
...
// // clang-format on
// for (uint8 i = 0; i < static_cast<uint8>(std::size(tuples)); i++)
...
// auto* company = CompanyManager::get(CompanyId(self.number));
// orphan member: Colour selectedColour;
...
// uint16 y = self.y + 62;
// for (uint8 i = 0; i < static_cast<uint8>(std::size(ExpenditureLabels)); i++)
...
// orphan member: FormatArguments args{};
// args.push<uint16>(getGameState().loanInterestRate);
...
// func If(0 any /* company->cash.var_04 < */ ) else
...
// args);
// y += 14;
// func DrawFinanceExpenditureColumn(drawingCtx Gfx::DrawingContext, x int16, y int16, columnIndex uint8, company Company) currency48_t
// auto tr = Gfx::TextRenderer(drawingCtx);
// currency48_t sum = 0;
...
// int16 y = 47 - self.widgets[widx::scrollview].top + 14;
// for (uint8 i = 0; i < static_cast<uint8>(ExpenditureType::Count); i++)
// // Add zebra stripes to even labels.
// if (i % 2 == 0)
// auto colour = Colours::getShade(self.getColour(WindowColour::secondary).c(), 6);
...
// y += 10;
// const auto company = CompanyManager::get(CompanyId(self.number));
// uint32 curYear = getCurrentYear();
// uint8 expenditureYears = std::min<uint8>(company->numExpenditureYears, kExpenditureHistoryCapacity);
// // Paint years on top of scroll area.
// int16 x = 132 - self.widgets[widx::scrollview].left + expenditureColumnWidth;
// for (auto i = 0; i < expenditureYears; i++)
// y = 46 - self.widgets[widx::scrollview].top;
// uint16 columnYear = curYear - (expenditureYears - i) + 1;
// uint8 columnIndex = expenditureYears - i - 1;
...
// const auto x = std::max<int16>(0, self.scrollAreas[0].contentOffsetX);
...
// self.scrollAreas[0].contentOffsetX = std::min<int16>(x, newOffset);
...
// self.invalidate();
// // 0x0043386F
// func GetScrollSize(self Window, scrollIndex [[maybe_unused]] uint32, scrollWidth int32, scrollHeight [[maybe_unused]] int32)
// const auto& company = CompanyManager::get(CompanyId(self.number));
...
// uint8 numPrinted = 0;
// const auto company = CompanyManager::get(CompanyId(self.number));
// for (uint8 i = 0; i < static_cast<uint8>(std::size(company->cargoDelivered)); i++)
...
// continue;
// cargoHeight += 10;
// const uint16 kWindowHeight = std::max<int16>(cargoHeight, 50) + 62;
...
// uint16 years = Scenario::getObjectiveProgress().completedChallengeInMonths / 12;
// uint16 months = Scenario::getObjectiveProgress().completedChallengeInMonths % 12;
// orphan member: FormatArguments args{};
// args.push(years);
// args.push(months);
...
// return;
// orphan member: FormatArguments args{};
// args.push<uint16>(playerCompany->challengeProgress);
...
// uint16 monthsLeft = Scenario::getObjective().timeLimitYears * 12 - Scenario::getObjectiveProgress().monthsInChallenge;
// uint16 years = monthsLeft / 12;
// uint16 months = monthsLeft % 12;
// orphan member: FormatArguments args{};
// args.push(years);
// args.push(months);
...
// static []<const Widget> widgetCollectionsByTabId[] = {
// Status::widgets,
// Details::widgets,
...
// const uint32 image = Gfx::recolour(competitor->images[enumValue(company->ownerEmotion)], company->mainColours.primary);
// const uint16 x = self->x + self->widgets[Common::widx::company_select].left + 1;
// const uint16 y = self->y + self->widgets[Common::widx::company_select].top + 1;
// drawingCtx.drawImage(x, y, image);
// // 0x00434413
// func DrawTabs(self Window, drawingCtx Gfx::DrawingContext)
// auto skin = ObjectManager::get<InterfaceSkinObject>();
// // Status tab
// const uint32 imageId = skin->img + InterfaceSkin::ImageIds::tab_company;
// Widget::drawTab(self, drawingCtx, imageId, widx::tab_status);
// // Details tab
// const uint32 imageId = Gfx::recolour(skin->img + InterfaceSkin::ImageIds::tab_company_details, self.getColour(WindowColour::primary).c());
// Widget::drawTab(self, drawingCtx, imageId, widx::tab_details);
// // Colour scheme tab
// static constexpr uint32 colourSchemeTabImageIds[] = {
...
// // Cargo delivered tab
// static constexpr uint32 cargoDeliveredTabImageIds[] = {
