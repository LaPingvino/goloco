...
// // Challenge tab
// static constexpr uint32 challengeTabImageIds[] = {
...
// // Companies tab
// const uint32 imageId = skin->img + InterfaceSkin::ImageIds::tab_companies;
// Widget::drawTab(self, drawingCtx, imageId, widx::tab_companies);
// // Finances tab
// static constexpr uint32 financesTabImageIds[] = {
...
// const uint32 imageId = skin->img + InterfaceSkin::ImageIds::tab_scenario_details;
...
// SKIPPED CONSTRUCTOR: const MaxCargoObjects uint8 = uint8(ObjectManager.getMaxObjects(ObjectType.cargo))

// static int16 cargoByDropdownIndex[kMaxCargoObjects] = { -1 };
// // 0x0043FD51
...
// uint32 stepSize = Input::getClickRepeatStepSize();
// switch (Scenario::getObjective().type)
// case Scenario::ObjectiveType::companyValue:
// Scenario::getObjective().companyValue = std::max<uint32>(Math::Bound::sub(Scenario::getObjective().companyValue, 100'000 * stepSize), Scenario::kMinObjectiveCompanyValue);
// break;
// case Scenario::ObjectiveType::vehicleProfit:
// Scenario::getObjective().monthlyVehicleProfit = std::max<uint32>(Math::Bound::sub(Scenario::getObjective().monthlyVehicleProfit, 1'000 * stepSize), Scenario::kMinObjectiveMonthlyProfitFromVehicles);
// break;
// case Scenario::ObjectiveType::performanceIndex:
// Scenario::getObjective().performanceIndex = std::max<uint8>(Scenario::getObjective().performanceIndex - 5, Scenario::kMinObjectivePerformanceIndex);
// break;
// case Scenario::ObjectiveType::cargoDelivery:
// stepSize *= 100;
// // Round off cargo to the nearest multiple of the step size.
// uint32 cargoFactor = Math::Bound::sub(Scenario::getObjective().deliveredCargoAmount, stepSize) / stepSize;
// uint32 newDeliveredCargoAmount = cargoFactor * stepSize;
// Scenario::getObjective().deliveredCargoAmount = std::max<uint32>(newDeliveredCargoAmount, Scenario::kMinObjectiveDeliveredCargo);
// break;
// self.invalidate();
// break;
...
// switch (Scenario::getObjective().type)
// case Scenario::ObjectiveType::companyValue:
// Scenario::getObjective().companyValue = std::min<uint32>(Math::Bound::add(Scenario::getObjective().companyValue, 100'000 * stepSize), Scenario::kMaxObjectiveCompanyValue);
// break;
// case Scenario::ObjectiveType::vehicleProfit:
// Scenario::getObjective().monthlyVehicleProfit = std::min<uint32>(Math::Bound::add(Scenario::getObjective().monthlyVehicleProfit, 1'000 * stepSize), Scenario::kMaxObjectiveMonthlyProfitFromVehicles);
// break;
// case Scenario::ObjectiveType::performanceIndex:
// Scenario::getObjective().performanceIndex = std::min<uint8>(Scenario::getObjective().performanceIndex + 5, Scenario::kMaxObjectivePerformanceIndex);
// break;
// case Scenario::ObjectiveType::cargoDelivery:
// stepSize *= 100;
// // Round off cargo to the nearest multiple of the step size.
// uint32 cargoFactor = Math::Bound::add(Scenario::getObjective().deliveredCargoAmount, stepSize) / stepSize;
// uint32 newDeliveredCargoAmount = cargoFactor * stepSize;
// Scenario::getObjective().deliveredCargoAmount = std::min<uint32>(newDeliveredCargoAmount, Scenario::kMaxObjectiveDeliveredCargo);
// break;
// self.invalidate();
// break;
// case widx::objective_cargo_btn:
// uint16 numCargoObjects = 0;
// for (uint16 cargoIdx = 0; cargoIdx < kMaxCargoObjects; cargoIdx++)
...
// Scenario::getObjective().timeLimitYears = std::max<uint8>(Scenario::getObjective().timeLimitYears - 1, Scenario::kMinObjectiveYearLimit);
// self.invalidate();
// break;
// case widx::time_limit_value_up:
// Scenario::getObjective().timeLimitYears = std::min<uint8>(Scenario::getObjective().timeLimitYears + 1, Scenario::kMaxObjectiveYearLimit);
// self.invalidate();
// break;
// // 0x0043FCED
...
// switch (Scenario::getObjective().type)
// case Scenario::ObjectiveType::companyValue:
// args.push<int32>(Scenario::getObjective().companyValue);
// self.widgets[widx::objective_value].text = StringIds::challenge_monetary_value;
// break;
// case Scenario::ObjectiveType::vehicleProfit:
// args.push<int32>(Scenario::getObjective().monthlyVehicleProfit);
// self.widgets[widx::objective_value].text = StringIds::challenge_monetary_value;
// break;
// case Scenario::ObjectiveType::performanceIndex:
// args.push<int16>(Scenario::getObjective().performanceIndex * 10);
// self.widgets[widx::objective_value].text = StringIds::challenge_performance_index;
// break;
// case Scenario::ObjectiveType::cargoDelivery:
// args.push<int32>(Scenario::getObjective().deliveredCargoAmount);
...
// args2.push<uint16>(Scenario::getObjective().timeLimitYears);
...
// namespace Companies
	...
	Max_competing_companies Widx = 8
...
// CompanyManager::setMaxCompetingCompanies(std::min<uint8>(CompanyManager::getMaxCompetingCompanies() + 1, Scenario::kMaxCompetingCompanies));
// self.invalidate();
// break;
...
// CompanyManager::setCompetitorStartDelay(std::min<uint8>(CompanyManager::getCompetitorStartDelay() + 1, Scenario::kMaxCompetitorStartDelay));
// self.invalidate();
// break;
// case widx::preferred_intelligence_btn:
...
// uint16 targetVehicle = static_cast<uint16>(widgetIndex - widx::competitor_forbid_trains);
// uint16 newForbiddenVehicles = state.forbiddenVehiclesCompetitors ^ (1 << targetVehicle);
...
// uint16 targetVehicle = static_cast<uint16>(widgetIndex - widx::player_forbid_trains);
// uint16 newForbiddenVehicles = state.forbiddenVehiclesPlayers ^ (1 << targetVehicle);
// // TODO(avgeffen): Add a constant for this mask.
// if (newForbiddenVehicles != 0b111111)
...
// args.push<uint16>(CompanyManager::getMaxCompetingCompanies());
// auto args = FormatArguments(self.widgets[widx::delay_before_competing_companies_start].textArgs);
// args.push<uint16>(CompanyManager::getCompetitorStartDelay());
// auto& state = getGameState();
...
// CompanyManager::setStartingLoanSize(std::max<int16>(CompanyManager::getStartingLoanSize() - 50, Scenario::kMinStartLoanUnits));
// break;
// case widx::starting_loan_up:
// CompanyManager::setStartingLoanSize(std::min<uint16>(CompanyManager::getStartingLoanSize() + 50, Scenario::kMaxStartLoanUnits));
...
// CompanyManager::setMaxLoanSize(std::max<int16>(CompanyManager::getMaxLoanSize() - 50, Scenario::kMinLoanSizeUnits));
// if (CompanyManager::getStartingLoanSize() > CompanyManager::getMaxLoanSize())
// CompanyManager::setStartingLoanSize((CompanyManager::getMaxLoanSize()));
// break;
// case widx::max_loan_size_up:
// CompanyManager::setMaxLoanSize(std::min<uint16>(CompanyManager::getMaxLoanSize() + 50, Scenario::kMaxLoanSizeUnits));
// break;
// case widx::loan_interest_rate_down:
// state.loanInterestRate = std::max<int16>(state.loanInterestRate - 1, Scenario::kMinLoanInterestUnits);
// break;
// case widx::loan_interest_rate_up:
// state.loanInterestRate = std::min<uint16>(state.loanInterestRate + 1, Scenario::kMaxLoanInterestUnits);
// break;
// self.invalidate();
// func OnMouseUp(self Window, widgetIndex WidgetIndex_t, id [[maybe_unused]] WidgetId)
// switch (widgetIndex)
...
// uint32 loanSizeInCurrency = CompanyManager::getInflationAdjustedStartingLoan();
// args.push<uint32>(loanSizeInCurrency);
...
// args.push(static_cast<uint32>(maxLoanSizeInCurrency));
// auto args = FormatArguments(self.widgets[widx::loan_interest_rate].textArgs);
// auto& state = getGameState();
// args.push<uint32>(state.loanInterestRate);
// static constexpr WindowEventList kEvents = {
// .onMouseUp = onMouseUp,
// .onMouseDown = onMouseDown,
// .onUpdate = Common::update,
...
// args.rewind();
// args.push(stex->name);
// const int16 xPos = window.x + 10;
// int16 yPos = window.y + widgets[widx::change_name_btn].top + 1;
// int16 width = widgets[widx::change_name_btn].left - 20;
// auto point = Point(xPos, yPos);
// tr.drawStringLeftClipped(point, width, Colour::black, StringIds::scenario_name_stringid, args);
// const int16 xPos = window.x + 10;
// int16 yPos = window.y + widgets[widx::scenario_group].top + 1;
// auto point = Point(xPos, yPos);
// tr.drawStringLeft(point, Colour::black, StringIds::scenario_group);
// const int16 xPos = window.x + 10;
// int16 yPos = window.y + widgets[widx::change_details_btn].top + 1;
// auto point = Point(xPos, yPos);
...
// orphan member: return kChallengeWindowSize;
// func If(widx::tab_companies widgetIndex ==) else
// orphan member: return kCompaniesWindowSize;
// else
// orphan member: return kOtherWindowSize;
// }();
