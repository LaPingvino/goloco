...
// // Rendering tab
// static constexpr uint32 kRedenderingTabImageIds[] = {
...
// static constexpr uint32 globe_tab_ids[] = {
...
// func MakeCommonWidgets(windowSize Ui::Size, windowCaptionId StringId) any
TabWidth = 31 // auto
// return makeWidgets(
// Widgets::Frame({ 0, 0 }, windowSize, WindowColour::primary),
// Widgets::Caption({ 1, 1 }, { (uint16)(windowSize.width - 2), 13 }, Widgets::Caption::Style::whiteText, WindowColour::primary, windowCaptionId),
// Widgets::ImageButton({ (int16)(windowSize.width - 15), 2 }, { 13, 13 }, WindowColour::primary, ImageIds::close_button, StringIds::tooltip_close_window),
...
// auto selection = static_cast<uint16>(Config::get().display.mode);
...
// Dropdown::add(i, StringIds::dropdown_stringid, { StringIds::display_resolution_dropdown_format, (uint16)resolutions[i].width, (uint16)resolutions[i].height });
...
// Dropdown::setItemSelected((int16)i);
// // 0x004C00F4
...
// args.push<uint16>(resolution.width);
// args.push<uint16>(resolution.height);
// // Scale.
// auto args = FormatArguments(self.widgets[Widx::display_scale].textArgs);
// args.push<int32>(Config::get().scaleFactor * 100);
...
SoundGroupOffset = 49                                        // auto
SoundGroupLastItemOffset = 30                                // auto
SoundGroupHeight = kSoundGroupLastItemOffset + 19            // auto
MusicGroupOffset = kSoundGroupOffset + kSoundGroupHeight + 4 // auto
MusicGroupLastItemOffset = 73                                // auto
MusicGroupHeight = kMusicGroupLastItemOffset + 19            // auto
...
// func If(SceneManager::isTitleMode() else
// auto& cfg = Config::get();
...
// func If(Jukebox::isMusicPlaying() else
...
// int16 x = 90 + (Config::get().audio.mainVolume / 32);
...
// Dropdown::setItemSelected((int16)currentDevice);
// // 0x004C04CA
// func AudioDeviceDropdown(self Window, itemIndex int16)
// if (itemIndex != -1)
...
// uint8 numLanguages = static_cast<uint8>(lds.size());
...
// string& current_language = Config::get().language;
// for (uint8 index = 1; index < numLanguages; index++)
// auto& ld = lds[index];
...
// Dropdown::setItemSelected(static_cast<uint8>(Config::get().measurementFormat));
// // 0x004C0FB3
...
// orphan member: FormatArguments args{ self.widgets[Widx::labelPreferredOwnerName].textArgs };
...
// orphan member: FormatArguments args{ self.widgets[Widx::labelOwnerFace].textArgs };
...
// orphan member: StringId stringId;
// switch (freq)
// case 0:
...
// Dropdown::add(2, StringIds::dropdown_stringid, { StringIds::autosave_every_x_months, static_cast<uint32>(3) });
// Dropdown::add(3, StringIds::dropdown_stringid, { StringIds::autosave_every_x_months, static_cast<uint32>(6) });
// Dropdown::add(4, StringIds::dropdown_stringid, { StringIds::autosave_every_x_months, static_cast<uint32>(12) });
