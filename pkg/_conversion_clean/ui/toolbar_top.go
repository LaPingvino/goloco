...
// static uint8 _lastPortOption; // 0x009C870D
...
// );
type LoadSaveDropdownId int

const (
func init() {
	LoadGame LoadSaveDropdownId = iota
}
...
// uint16 highlightedItem = 0;
// for (auto i = 0u; i < _railroadMenuObjects.size(); i++)
// orphan member: uint32 objImage;
// orphan member: StringId objStringId;
...
// if (itemIndex == -1)
// return;
// uint8 objIndex = _railroadMenuObjects[itemIndex];
...
// uint16 availableVehicles = company->availableVehicles;
// auto companyColour = CompanyManager::getPlayerCompanyColour();
// auto interface = ObjectManager::get<InterfaceSkinObject>();
// uint8 ddIndex = 0;
// for (uint8 vehicleType = 0; vehicleType < vehicleTypeCount; vehicleType++)
...
// uint32 vehicle_image = Gfx::recolour(interface_param.buildImage, companyColour);
...
// auto companyColour = CompanyManager::getPlayerCompanyColour();
// auto interface = ObjectManager::get<InterfaceSkinObject>();
// uint16 vehicle_counts[vehicleTypeCount]{ 0 };
// for (auto* v : VehicleManager::VehicleList())
// if (v->owner != player_company_id)
// continue;
// if (v->has38Flags(Vehicles::Flags38::isGhost))
// continue;
// vehicle_counts[static_cast<uint8>(v->vehicleType)]++;
// uint8 ddIndex = 0;
// for (uint16 vehicleType = 0; vehicleType < vehicleTypeCount; vehicleType++)
// if ((availableVehicles & (1 << vehicleType)) == 0)
// continue;
// auto& interfaceParam = VehicleTypeInterfaceParameters.at(static_cast<VehicleType>(vehicleType));
// uint32 vehicleImage = Gfx::recolour(interfaceParam.image, companyColour);
// uint16 vehicle_count = vehicle_counts[vehicleType];
// // TODO: replace with locale-based plurals.
// orphan member: StringId vehicleStringId;
// if (vehicle_count == 1)
...
// ddIndex++;
// Dropdown::showBelow(window, widgetIndex, ddIndex, 25, (1 << 6));
// Dropdown::setHighlightedItem(static_cast<uint8>(getGameState().lastVehicleType));
// // 0x0043ACEF
...
// auto interface = ObjectManager::get<InterfaceSkinObject>();
// uint32 sprite_base = interface->img;
...
// uint32 x = window.widgets[Common::Widx::railroad_menu].left + window.x;
// uint32 y = window.widgets[Common::Widx::railroad_menu].top + window.y;
// uint32 fg_image = 0;
// // Figure out what icon to show on the button face.
// uint8 ebx = getGameState().lastRailroadOption;
// if ((ebx & (1 << 7)) != 0)
// ebx = ebx & ~(1 << 7);
...
// auto interface = ObjectManager::get<InterfaceSkinObject>();
// uint32 bg_image = Gfx::recolour(interface->img + InterfaceSkin::ImageIds::toolbar_empty_transparent, window.getColour(WindowColour::tertiary).c());
// y--;
...
// uint32 x = window.widgets[Common::Widx::vehicles_menu].left + window.x;
// uint32 y = window.widgets[Common::Widx::vehicles_menu].top + window.y;
// static constexpr uint32 button_face_image_ids[] = {
...
// auto interface = ObjectManager::get<InterfaceSkinObject>();
// uint32 fg_image = Gfx::recolour(interface->img + button_face_image_ids[static_cast<uint8>(getGameState().lastVehicleType)], companyColour);
// uint32 bg_image = Gfx::recolour(interface->img + InterfaceSkin::ImageIds::toolbar_empty_transparent, window.getColour(WindowColour::quaternary).c());
// y--;
...
// drawingCtx.drawImage(x, y, bg_image);
// uint32 x = window.widgets[Common::Widx::build_vehicles_menu].left + window.x;
// uint32 y = window.widgets[Common::Widx::build_vehicles_menu].top + window.y;
// static constexpr uint32 kBuildVehicleImages[] = {
...
// // Figure out what icon to show on the button face.
// auto interface = ObjectManager::get<InterfaceSkinObject>();
// uint32 fg_image = Gfx::recolour(interface->img + kBuildVehicleImages[enumValue(getGameState().lastBuildVehiclesOption)], companyColour);
...
// const auto offsetWidget = [&window, refWidget](uint8 widgetIndex, uint8 index) {
...
// uint32 x = std::max(640, Ui::width()) - 1;
