...
func init() {
	...
	Vehicle_preview
// static constexpr uint32 kTrainTabImages[16]{
...
// static constexpr uint32 kAircraftTabImages[16]{
...
// static constexpr uint32 kBusTabImages[16]{
...
// static constexpr uint32 kTramTabImages[16]{
...
// static constexpr uint32 kTruckTabImages[16]{
...
// static constexpr uint32 kShipTabImages[16]{
...
// static uint16 _lastRefreshYear;
...
// static uint8 _cargoSupportedFilter = 0xFF;
// static uint32 _numTrackTypeTabs;    // 0x011364EC
// static int16 _numAvailableVehicles; // 0x01136268
// // Array of types if 0xFF then no type, flag (1<<7) as well
// static uint8 _trackTypesForTab[widxToTrackTypeTab(widx::tab_track_type_7) + 1];      // 0x011364F0
// static uint16 _availableVehicles[ObjectManager::getMaxObjects(ObjectType::vehicle)]; // 0x0113626A
// static int32 _buildTargetVehicle; // 0x011364E8; -1 for no target VehicleHead
ScrollRowHeight = [6]uint16{
...
// // 0x004C1AF7
// static Window* open(uint32 vehicleId, bool isTabId)
...
// tab += static_cast<uint8>(veh->vehicleType);
// else
...
// orphan member: return nullptr;
// window->currentTab = static_cast<uint8>(veh->vehicleType);
// else
...
// for (uint32 trackTypeTab = 0; trackTypeTab < _numTrackTypeTabs; trackTypeTab++)
...
// Window* openByVehicleObjectId(uint16 vehicleObjectId)
...
// for (uint8 i = 0; i < _numTrackTypeTabs && i < std::size(_trackTypesForTab); ++i)
...
// for (uint16 vehicleObjIndex = 0; vehicleObjIndex < ObjectManager::getMaxObjects(ObjectType::vehicle); ++vehicleObjIndex)
...
// continue;
// string_view pattern = inputSession.buffer;
// if (!pattern.empty())
// const string_view name = StringManager::getString(vehicleObj->name);
// if (!contains(name, pattern))
// continue;
// if (trackType != 0xFF)
// uint8 sanitisedTrackType = trackType;
// if (trackType & (1 << 7))
...
// const string_view str1 = StringManager::getString(item1.name);
// const string_view str2 = StringManager::getString(item2.name);
// return str1 < str2;
// });
...
// _numAvailableVehicles = static_cast<int16>(buildableVehicles.size());
...
// for (auto i = (int32)WindowManager::count() - 1; i >= 0; i--)
...
// uint8 trackType = _trackTypesForTab[window->currentSecondaryTab];
...
// uint16* src = _availableVehicles;
// int16* dest = window->rowInfo;
...
// auto minWidth = std::max<uint16>(_numTrackTypeTabs * 31 + 195, 380);
...
// // 0x4C37B9
// func GetScrollSize(window Ui::Window, scrollIndex [[maybe_unused]] uint32, scrollWidth [[maybe_unused]] int32, scrollHeight int32) 
// scrollHeight = window.var_83C * window.rowHeight;
// // 0x4C384B
// func OnScrollMouseDown(window Ui::Window, x [[maybe_unused]] int16, y int16, scroll_index uint8) 
...
// sub_4B92A5(&window);
// // 0x4C3802
// func OnScrollMouseOver(window Ui::Window, x [[maybe_unused]] int16, y int16, scroll_index uint8) 
// if (scroll_index != scrollIdx::vehicle_selection)
// return;
// auto scrollItem = y / window.rowHeight;
// int16 item = -1;
...
// // 0x4C37CB
// static Ui::CursorId cursor(Window& window, WidgetIndex_t widgetIdx, [[maybe_unused]] const WidgetId id, [[maybe_unused]] int16 xPos, int16 yPos, Ui::CursorId fallback)
...
// args);
// if (vehicleObj->obsolete != 0 && vehicleObj->obsolete != std::numeric_limits<uint16>::max())
// orphan member: FormatArguments args{};
...
NormalHoverRowColour = enumValue(ExtColour.unk30) // auto
...
// int16 half = (window.rowHeight - 22) / 2;
// auto x = drawVehicleInline(drawingCtx, vehicleType, CompanyManager::getControllingId(), { 0, static_cast<int16>(y + half) });
...
// if (window.rowHover == -1)
// break;
// uint8 yaw = Ui::WindowManager::getVehiclePreviewRotationFrameYaw();
// uint8 roll = Ui::WindowManager::getVehiclePreviewRotationFrameRoll();
...
// *(reinterpret_cast<uint32*>(buffer)) = cargoObj->unitInlineSprite;
// buffer += 4;
// *buffer++ = '\0';
// orphan member: FormatArguments args{};
...
// uint8 trackType = _trackTypesForTab[trackTab] & ~(1 << 7);
...
// uint32 image = Gfx::recolour(skin->img + tab.imageIds[frameNo], companyColour);
...
// bottom = top + 27;
// drawingCtx.fillRect(left, top, right, bottom, Colours::getShade(window.getColour(WindowColour::secondary).c(), 7), Gfx::RectFlags::none);
// for (uint32 tab = 0; tab < _numTrackTypeTabs; ++tab)
