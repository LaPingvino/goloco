...
// auto town = TownManager::get(TownId(self.number));
// const uint32 ebx = (town->numBuildings >> 3) + 5;
// const int16 currentYear = getCurrentYear();
// int16 tempYear = currentYear - 51;
// setCurrentYear(tempYear);
// for (uint8 i = 8; i > 0; i--)
// for (uint32 j = ebx; j > 0; j--)
...
// uint8 historyEntry = std::min<uint8>(town->population / 50, 255);
// town->history[town->historySize - 1] = historyEntry;
// // Play construction sound at the town centre.
// int16 tileZ = World::TileManager::getHeight({ town->x, town->y }).landHeight;
...
// int32 yTick = town->historyMinPopulation;
// for (int16 yPos = self.height - 57; yPos >= 14; yPos -= 20)
// orphan member: FormatArguments args{};
// args.push(yTick);
// const uint16 xPos = 39;
...
// int16 year = getCurrentYear();
// int8_t yearSkip = 0;
// for (uint8 i = town->historySize - 1; i > 0; i--)
// const uint16 xPos = 41 + i;
// const uint16 yPos = 2;
...
// uint16 yPos1 = -town->history[i] + (self.height - 57);
// uint16 yPos2 = -town->history[i + 1] + (self.height - 57);
...
// point.y += 14;
// auto town = TownManager::get(TownId(self.number));
// for (uint8 i = 0; i < std::size(town->companyRatings); i++)
// if ((town->companiesWithRating & (1 << i)) == 0)
// continue;
// int16 rating = (std::clamp<int16>(town->companyRatings[i], -1000, 1000) + 1000) / 20;
// orphan member: StringId rank{};
// if (rating >= 70)
// rank = StringIds::town_rating_excellent;
// func If(60 rating >=) else
// rank = StringIds::town_rating_good;
// func If(50 rating >=) else
// rank = StringIds::town_rating_average;
// func If(25 rating >=) else
...
// // Population tab
// static constexpr uint32 populationTabImageIds[] = {
...
// uint32 imageId = Gfx::recolour(skin->img, self.getColour(WindowColour::secondary).c());
...
// // Company ratings tab
// static constexpr uint32 ratingsTabImageIds[] = {
