...
const RawTransparent byte = ' '
const RawWhite byte = '.'
const RawBlack byte = 'X'
...
type Cursor struct {
	X int
	Y int
	// uint8 data[encodedCursorSize]{};
	// uint8 mask[encodedCursorSize]{};
	// static Cursor blank;
	...
	// method: constexpr Cursor(int x, int y, string_view bitmap);
}

// constexpr Cursor::Cursor(int x, int y, string_view bitmap)
// : x(x)
// , y(y)
// assert(bitmap.length() == rawCursorSize);
// this->x = x;
// this->y = y;
// orphan member: uint8 curBit{};
// orphan member: uint16 curPixel{};
// uint8 dataByte{}, maskByte{};
