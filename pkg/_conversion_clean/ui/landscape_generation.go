...
// SKIPPED CONSTRUCTOR: const MaxLandObjects int = ObjectManager.getMaxObjects(ObjectType.land)
...
// // Options tab
// static constexpr uint32 optionTabImageIds[] = {
...
// const uint32 imageId = land->mapPixelImage + OpenLoco::Land::ImageIds::toolbar_terraform_land;
// Widget::drawTab(self, drawingCtx, imageId, widx::tab_land);
// // Water tab
// const auto waterObj = ObjectManager::get<WaterObject>();
// uint32 imageId = waterObj->image + OpenLoco::Water::ImageIds::kToolbarTerraformWater;
...
// // Forest tab
// const uint32 imageId = skin->img + InterfaceSkin::ImageIds::toolbar_menu_plant_trees;
// Widget::drawTab(self, drawingCtx, imageId, widx::tab_forests);
// // Towns tab
// const uint32 imageId = skin->img + InterfaceSkin::ImageIds::toolbar_menu_towns;
// Widget::drawTab(self, drawingCtx, imageId, widx::tab_towns);
// // Industries tab
// const uint32 imageId = skin->img + InterfaceSkin::ImageIds::toolbar_menu_industries;
...
// args.push<uint16>(options.scenarioStartYear);
...
// orphan member: FormatArguments args{ widget.textArgs };
// args.push<uint16>(options.numTerrainSmoothingPasses);
// self.disabledWidgets |= (1 << widx::change_heightmap_btn);
// self.disabledWidgets &= ~((1 << widx::terrainSmoothingNum) | (1 << widx::terrainSmoothingNumUp) | (1 << widx::terrainSmoothingNumDown));
// self.disabledWidgets |= (1 << widx::browseHeightmapFile);
...
// Dropdown::setHighlightedItem(static_cast<uint8>(options.generator));
// break;
// // 0x0043DC58
...
// // 0x0043E01C
// func DrawScroll(window Ui::Window, drawingCtx Gfx::DrawingContext, scrollIndex [[maybe_unused]] uint32)
// const auto& rt = drawingCtx.currentRenderTarget();
// auto tr = Gfx::TextRenderer(drawingCtx);
// uint16 yPos = 0;
// for (uint16 i = 0; i < kMaxLandObjects; i++)
...
// continue;
// // Draw tile icon.
// const uint32 imageId = landObject->mapPixelImage + OpenLoco::Land::ImageIds::landscape_generator_tile_icon;
...
// yPos += kRowHeight;
// // 0x0043E2AC
// func GetScrollSize(window [[maybe_unused]] Ui::Window, scrollIndex [[maybe_unused]] uint32, scrollWidth [[maybe_unused]] int32, scrollHeight int32)
// scrollHeight = 0;
// for (uint16 i = 0; i < kMaxLandObjects; i++)
// auto landObject = ObjectManager::get<LandObject>(i);
// if (landObject == nullptr)
...
// Dropdown::setHighlightedItem(static_cast<uint8>(options.topographyStyle));
// break;
...
// return -1;
// // 0x0043E1CF
// func ScrollMouseDown(window Window, xPos int16, yPos int16, scrollIndex [[maybe_unused]] uint8)
// int16 landIndex = scrollPosToLandIndex(xPos, yPos);
// if (landIndex == -1)
// return;
...
// const int16 dropdownX = window.x + target.left + kLandDropdownLeft + 1;
// const int16 dropdownY = window.y + target.top + 6 + landIndex * kRowHeight - window.scrollAreas[0].contentOffsetY;
...
// args.push<uint16>(options.hillDensity);
// auto args = FormatArguments(window.widgets[widx::min_land_height].textArgs);
// args.push<uint16>(options.minLandHeight);
// window.widgets[widx::topography_style].text = topographyStyleIds[static_cast<uint8>(options.topographyStyle)];
...
// options.numberOfForests = std::min<int16>(options.numberOfForests + 10, Scenario::kMaxNumForests);
// break;
// case widx::number_of_forests_down:
// options.numberOfForests = std::max<int16>(Scenario::kMinNumForests, options.numberOfForests - 10);
// break;
// case widx::min_forest_radius_up:
// options.minForestRadius = std::min<int16>(options.minForestRadius + 1, Scenario::kMaxForestRadius);
...
// options.numberRandomTrees = std::min<int16>(options.numberRandomTrees + 25, Scenario::kMaxNumTrees);
// break;
// case widx::number_random_trees_down:
// options.numberRandomTrees = std::max<int16>(Scenario::kMinNumTrees, options.numberRandomTrees - 25);
// break;
...
// uint16 newNumTowns = std::min<uint16>(options.numberOfTowns + 1, Limits::kMaxTowns);
...
// uint16 newNumTowns = std::max<uint16>(Limits::kMinTowns - 1, options.numberOfTowns - 1);
// options.numberOfTowns = newNumTowns;
// window.invalidate();
// break;
...
// args.push<uint16>(Scenario::getOptions().numberOfTowns);
