...
// const uint16 numCharacters = static_cast<uint16>(inputSession.buffer.length());
// const uint16 maxNumCharacters = inputSession.inputLenLimit;
// orphan member: FormatArguments args{ self.widgets[Widx::charLimit].textArgs };
// args.push<uint16>(numCharacters);
// args.push<uint16>(maxNumCharacters);
// /**
// * 0x004CE75B
// *
// * @param window @<esi>
...
// args2.push(_message);
// args2.push(*reinterpret_cast<uint32*>(_formatArgs.data() + 8));
// args2.push(*reinterpret_cast<uint32*>(_formatArgs.data() + 12));
...
// func If(SDLK_ESCAPE charCode ==) else
// w.callOnMouseUp(Widx::close, w.widgets[Widx::close].id);
// orphan member: return true;
// func If(!Input::isFocused(w.type, w.number, Widx::input) else
