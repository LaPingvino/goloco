...
// static uint16 _mapTooltipTimeout; // 0x00523348
type Widx int
...
// int16 y = cursor.y + 15; // Normally, we'd display the tooltip 15 lower
// if (y > maxY)
// // If y is too large, the tooltip could be forced below the cursor if we'd just clamped y,
// // so we'll subtract a bit more
// y -= height + 19;
// auto width = 240;
// int16 x = width <= Ui::width() ? std::clamp(cursor.x - (width / 2), 0, Ui::width() - width) : 0;
