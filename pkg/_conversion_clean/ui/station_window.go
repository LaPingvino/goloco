...
// static constexpr Ui::Size kWindowSize = { 223, 136 };
	...
	Viewport Widx = Common.widx.content_begin
	Status_bar
	Centre_on_viewport
...
// auto station = StationManager::get(StationId(self.number));
// uint8 cargoTypeCount = 0;
// for (uint32 cargoId = 0; cargoId < kMaxCargoStats; cargoId++)
// auto& stats = station->cargoStats[cargoId];
// if (!stats.isAccepted())
// continue;
// *buffer++ = ' ';
// *buffer++ = ControlCodes::inlineSpriteStr;
// *(reinterpret_cast<uint32*>(buffer)) = ObjectManager::get<CargoObject>(cargoId)->unitInlineSprite;
// buffer += 4;
// cargoTypeCount++;
// if (cargoTypeCount == 0)
...
// orphan member: FormatArguments args{};
// args.push(cargoName);
// args.push<uint32>(cargo.quantity);
...
// );
// func VehicleStopsAtActiveStation(head VehicleHead, filterStationId StationId) bool
...
// for (uint32 posId = 0; posId < kMapSize; posId++)
...
// // Cargo ratings tab
// const uint32 imageId = skin->img + InterfaceSkin::ImageIds::tab_cargo_ratings;
...
// break;
// // clang-format off
// static constexpr std::pair<WidgetIndex_t, []<uint32, 8>> kTabAnimations[] = {
