...
const RenderedMapWidth int16 = kMapColumns * 2
const RenderedMapHeight int16 = kRenderedMapWidth
const RenderedMapSize int32 = kRenderedMapWidth * kRenderedMapHeight
...
// // 0x004FDC4C
// static []<Point, 4> kViewFrameOffsetsByRotation = { {
// { kMapColumns, 0 },
// { kRenderedMapWidth, kMapRows },
// { kMapColumns, kRenderedMapHeight },
// { 0, kMapRows },
// } };
var FlashColours = [256]PaletteIndex_t{
// []<PaletteIndex_t, 256> colours;
// std::fill(colours.begin(), colours.end(), PaletteIndex::black0);
// std::fill(colours.begin() + 10, colours.begin() + 14, PaletteIndex::blackB);
// orphan member: return colours;
// }(); // 0x004FDC5C
// static PaletteIndex_t* _mapPixels; // 0x00F253A8
// static PaletteIndex_t* _mapAltPixels;
// static []<uint16, 6> _vehicleTypeCounts = {};
// static uint32 _flashingItems;              // 0x00F253A4
// static uint32 _drawMapRowIndex;            // 0x00F253AC
// static uint8 _assignedIndustryColours[16]; // 0x00F253CE
// static uint8 _routeToObjectIdMap[19];      // 0x00F253DF
// static uint8 _routeColours[19];            // 0x00F253F2
// static uint8 _trackColours[8];             // 0x00F25404
// static uint8 _roadColours[8];              // 0x00F2540C
type Widx int
...
// );
// func MapWindowPosToLocation(pos Point) Pos2
...
// func LocationToMapWindowPos(pos Pos2) Point
// int32 x = pos.x;
// int32 y = pos.y;
// switch (getCurrentRotation())
// case 3:
...
// x /= kTileSize;
// y /= kTileSize;
// func Point(kMapColumns -x + y +, y x +) return
// // 0x0046B8E6
...
// // 0x0046C5E5
// func SetMapPixelsOverall(mapPtr PaletteIndex_t, mapAltPtr PaletteIndex_t, pos Pos2, delta Pos2) 
...
// _drawMapRowIndex = 0;
// // 0x0046C873
// func SetMapPixelsVehicles(mapPtr PaletteIndex_t, mapAltPtr PaletteIndex_t, pos Pos2, delta Pos2) 
// for (auto rowCountLeft = kMapColumns; rowCountLeft > 0; rowCountLeft--)
// // Coords shouldn't be at map edge
// if (!(pos.x > 0 && pos.y > 0 && pos.x < kMapWidth - kTileSize && pos.y < kMapHeight - kTileSize))
// pos += delta;
...
// // 0x004FB464
// // clang-format off
var IndustryColours = [31]PaletteIndex_t{
...
// // clang-format on
// // 0x0046C9A8
// func SetMapPixelsIndustries(mapPtr PaletteIndex_t, mapAltPtr PaletteIndex_t, pos Pos2, delta Pos2) 
// for (auto rowCountLeft = kMapColumns; rowCountLeft > 0; rowCountLeft--)
// // Coords shouldn't be at map edge
// if (!(pos.x > 0 && pos.y > 0 && pos.x < kMapWidth - kTileSize && pos.y < kMapHeight - kTileSize))
// pos += delta;
...
// _drawMapRowIndex = 0;
// // 0x0046CB68
// func SetMapPixelsRoutes(mapPtr PaletteIndex_t, mapAltPtr PaletteIndex_t, pos Pos2, delta Pos2) 
// for (auto rowCountLeft = kMapColumns; rowCountLeft > 0; rowCountLeft--)
// // Coords shouldn't be at map edge
// if (!(pos.x > 0 && pos.y > 0 && pos.x < kMapWidth - kTileSize && pos.y < kMapHeight - kTileSize))
// pos += delta;
...
// if (surfaceEl == nullptr)
// continue;
// uint8 terrainColour0{}, terrainColour1{};
// if (surfaceEl->water() == 0)
// const auto* landObj = ObjectManager::get<LandObject>(surfaceEl->terrain());
// const auto* landImage = Gfx::getG1Element(landObj->mapPixelImage);
...
// _drawMapRowIndex = 0;
// // 0x0046CD31
// func SetMapPixelsOwnership(mapPtr PaletteIndex_t, mapAltPtr PaletteIndex_t, pos Pos2, delta Pos2) 
// for (auto rowCountLeft = kMapColumns; rowCountLeft > 0; rowCountLeft--)
// // Coords shouldn't be at map edge
// if (!(pos.x > 0 && pos.y > 0 && pos.x < kMapWidth - kTileSize && pos.y < kMapHeight - kTileSize))
// pos += delta;
...
// owner = station->owner;
// func If(el.as<TrackElement>( auto trackEl =) else
// owner = trackEl->owner();
// func If(el.as<RoadElement>( auto roadEl =) else
// owner = roadEl->owner();
...
// orphan member: Pos2 pos{};
// orphan member: Pos2 delta{};
...
// // 0x0046D34D based on
// func SetHoverItem(self Window, y int16, index int) 
// orphan member: uint32 itemHover;
// if (y < 0)
// itemHover = (1 << index);
// else
// itemHover = 0;
...
// if (self->var_854 != 0)
// self->invalidate();
// static uint8 legendWidth = 100;
// static uint8 legendItemHeight = 10;
const OverallGraphKeySize uint8 = 6
// static []<size_t, 5> legendLengths = {
// kOverallGraphKeySize,
// std::size(_vehicleTypeCounts),
// ObjectManager::getMaxObjects(ObjectType::industry),
// 0,
// Limits::kMaxCompanies,
// func SetHoverItemTab(self Window, legendLeft int16, legendBottom int16) 
...
// if (y < 0)
// break;
// func If(legendItemHeight any /* cursorPos.y < static_cast<int16>(legendLengths[self->currentTab] */ ) else
// y = cursorPos.y;
...
// continue;
// func If(widx::tabOverall self->currentTab == (widx::tabOwnership -) else
...
// // 0x00F2541D
// static uint16 mapFrameNumber = 0;
// // 0x0046BA5B
...
// // 0x0046B9E7
// func GetScrollSize(self [[maybe_unused]] Window, scrollIndex [[maybe_unused]] uint32, scrollWidth int32, scrollHeight int32) 
// scrollWidth = kRenderedMapWidth;
// scrollHeight = kRenderedMapHeight;
// // 0x0046B9D4
// func MoveMainViewToMapView(pos Pos2) 
...
// // 0x0046B97C
// func ScrollMouseDown(self [[maybe_unused]] Window, x int16, y int16, scrollIndex [[maybe_unused]] uint8) 
...
// static constexpr uint32 kMapSpritesByRotation[] = {
...
// uint32 mapSprite = skin->img + kMapSpritesByRotation[WindowManager::getCurrentRotation()];
// Widget::drawTab(self, drawingCtx, mapSprite, widx::tabOverall);
// // tabVehicles,
// if (!(self.disabledWidgets & (1 << widx::tabVehicles)))
// static constexpr uint32 vehicleImageIds[] = {
...
// static constexpr uint32 routeImageIds[] = {
...
// // 0x0046D273
// func DrawGraphKeyOverall(self Window, drawingCtx Gfx::DrawingContext, x uint16, y uint16) 
...
// // 0x0046D379
// func DrawGraphKeyVehicles(self Window, drawingCtx Gfx::DrawingContext, x uint16, y uint16) 
// static constexpr StringId lineNames[] = {
...
// auto tr = Gfx::TextRenderer(drawingCtx);
// for (uint8 i = 0; i < std::size(_vehicleTypeCounts); i++)
// if (!(self->var_854 & (1 << i)) || !(mapFrameNumber & (1 << 2)))
...
// y += 10;
// // 0x0046D47F
// func DrawGraphKeyIndustries(self Window, drawingCtx Gfx::DrawingContext, x uint16, y uint16) 
// auto tr = Gfx::TextRenderer(drawingCtx);
// for (uint8 i = 0; i < ObjectManager::getMaxObjects(ObjectType::industry); i++)
...
// y += 10;
// // 0x0046D5A4
// func DrawGraphKeyRoutes(self Window, drawingCtx Gfx::DrawingContext, x uint16, y uint16) 
// auto tr = Gfx::TextRenderer(drawingCtx);
...
// y += 10;
// // 0x0046D6E1
// func DrawGraphKeyCompanies(self Window, drawingCtx Gfx::DrawingContext, x uint16, y uint16) 
// auto tr = Gfx::TextRenderer(drawingCtx);
...
// y += 10;
// // 0x0046D81F
// func FormatVehicleString(self Window, args FormatArguments) 
...
// int16 vehicleIndex = Numerics::bitScanForward(self->var_854);
// uint16 totalVehicleCount = 0;
...
// // 0x0046D87C
// func FormatIndustryString(self Window, args FormatArguments) 
// int16 industryIndex = Numerics::bitScanForward(self->var_854);
// if (industryIndex == -1)
...
// func If(industryObj->producesCargo() else
...
// y -= self.y;
// y += 14;
// y = std::max<uint16>(y, kMinWindowSize.height);
// self.minHeight = y;
// orphan member: FormatArguments args{};
// switch (self.currentTab + widx::tabOverall)
// case widx::tabOverall:
...
// // 0x0046BF0F based on
// func DrawVehicleOnMap(drawingCtx Gfx::DrawingContext, vehicle Vehicles::VehicleBase, colour uint8) 
...
// // 0x0046C294
// static std::pair<Point, Point> drawRouteLine(Gfx::DrawingContext& drawingCtx, Point startPos, Point endPos, Pos2 stationPos, uint8 colour)
...
// static std::optional<uint8> getRouteColour(Vehicles::Vehicle train)
// orphan member: uint8 colour;
...
// func If(VehicleType::ship train.head->vehicleType ==) else
// colour = 139;
// auto index = Numerics::bitScanForward(_flashingItems);
// if (index != -1)
...
// else
// return std::nullopt;
// orphan member: return colour;
// // 0x0046C18D
// func DrawRoutesOnMap(drawingCtx Gfx::DrawingContext, train Vehicles::Vehicle) 
...
// // 0x0046C426
// func GetVehicleColour(widgetIndex WidgetIndex_t, train Vehicles::Vehicle, car Vehicles::Car) uint8
...
// continue;
// auto vehicleType = train.head->vehicleType;
// _vehicleTypeCounts[static_cast<uint8>(vehicleType)] = _vehicleTypeCounts[static_cast<uint8>(vehicleType)] + 1;
// // 0x0046BE6E, 0x0046C35A
// func DrawVehiclesOnMap(drawingCtx Gfx::DrawingContext, widgetIndex WidgetIndex_t) 
// for (auto* vehicle : VehicleManager::VehicleList())
// Vehicles::Vehicle train(*vehicle);
// if (train.head->has38Flags(Vehicles::Flags38::isGhost))
// continue;
...
// func DrawRectOnMap(drawingCtx Gfx::DrawingContext, left int16, top int16, right int16, bottom int16, colour uint8, flags Gfx::RectFlags) 
// if (left > right)
// std::swap(left, right);
// if (top > bottom)
// std::swap(top, bottom);
...
// // 0x0046BE51
// func DrawViewOnMap(drawingCtx Gfx::DrawingContext, left int16, top int16, right int16, bottom int16) 
// left /= 32;
// top /= 16;
// right /= 32;
// bottom /= 16;
...
// // 0x0046BE34
// func DrawViewCornersOnMap(drawingCtx Gfx::DrawingContext, left int16, top int16, leftOffset int16, topOffset int16, rightOffset int16, bottomOffset int16) 
// left /= 32;
// top /= 16;
// left += kViewFrameOffsetsByRotation[getCurrentRotation()].x;
// top += kViewFrameOffsetsByRotation[getCurrentRotation()].y;
...
// // 0x0046BAD5
// func DrawViewportPosition(drawingCtx Gfx::DrawingContext) 
// auto window = WindowManager::getMainWindow();
// if (window == nullptr)
// return;
...
// drawViewCornersOnMap(drawingCtx, left, top, 0, -cornerSize, 0, 0);
// // 0x0046C481
// func DrawTownNames(drawingCtx Gfx::DrawingContext) 
// auto tr = Gfx::TextRenderer(drawingCtx);
...
// // 0x0046B806
// func DrawScroll(self Window, drawingCtx Gfx::DrawingContext, scrollIndex [[maybe_unused]] uint32) 
...
// // 0x0046D0C3
// func CheckIndustryColours(colour PaletteIndex_t, colourMask uint32) uint32
// for (auto i = 0; i < 31; i++)
...
// if (diff <= 2)
// colourMask &= ~(1U << i);
// orphan member: return colourMask;
// // 0x0046CFF0
// func AssignIndustryColours() 
// uint32 availableColours = 0x7FFFFFFF;
...
// auto i = 0U;
// auto assignColour = [&i, &availableColours](uint8 id) {
...
// size.width = std::clamp<uint16>(size.width, 350, Ui::width());
// size.height = std::clamp<uint16>(size.height, 272, Ui::height() - 56);
...
// const int16 vpCentreX = ((viewport->viewWidth / 2) + viewport->viewX) / 32;
// const int16 vpCentreY = ((viewport->viewHeight / 2) + viewport->viewY) / 16;
// auto& widget = window->widgets[widx::scrollview];
// const int16 miniMapWidth = widget.width() - ScrollView::kScrollbarSize;
// const int16 miniMapHeight = widget.height() - ScrollView::kScrollbarSize;
// const int16 visibleMapWidth = window->scrollAreas[0].contentWidth - miniMapWidth;
// const int16 visibleMapHeight = window->scrollAreas[0].contentHeight - miniMapHeight;
// auto& offset = kViewFrameOffsetsByRotation[getCurrentRotation()];
// int16 centreX = std::max(vpCentreX + offset.x - (miniMapWidth / 2), 0);
// int16 centreY = std::max(vpCentreY + offset.y - (miniMapHeight / 2), 0);
