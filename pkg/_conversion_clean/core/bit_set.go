...
// SKIPPED CONSTRUCTOR: const NumBits int = byteAlignBits<TNumBits>()
...
// static_assert(computeBlockSize<1>() == sizeof(uint8));
// static_assert(computeBlockSize<4>() == sizeof(uint8));
// static_assert(computeBlockSize<8>() == sizeof(uint8));
// static_assert(computeBlockSize<9>() == sizeof(uint16));
// static_assert(computeBlockSize<14>() == sizeof(uint16));
// static_assert(computeBlockSize<16>() == sizeof(uint16));
// static_assert(computeBlockSize<18>() == sizeof(uint32));
// static_assert(computeBlockSize<31>() == sizeof(uint32));
...
// SKIPPED C++ SYNTAX: type Value_type = any /* typename StorageBlockType<computeBlockSize<TBitSize>()>::value_type */ 
// template<size_t TBitSize>
type BitSet struct {
// SKIPPED C++ SYNTAX: type StorageBlockType = any /* typename Detail::BitSet::storage_block_type_aligned<kByteAlignedBitSize>::value_type */ 
type BlockType = StorageBlockType
type Storage = any /* []<BlockType, kBlockCount> */ 
...
// SKIPPED CONSTRUCTOR: const BitSetByteAlignedBitSize int = Detail.BitSet.byteAlignBits<TBitSize>()
// SKIPPED CONSTRUCTOR: const BitSetBlockByteSize int = sizeof(StorageBlockType)
const BitSetBlockBitSize int = kBlockByteSize * Detail.BitSet.kBitsPerByte
// SKIPPED CONSTRUCTOR: const BitSetBlockCount int = Detail.BitSet.computeBlockCount<kByteAlignedBitSize, kBlockByteSize>()
...
// SKIPPED CONSTRUCTOR: const BitSetBlockValueMask StorageBlockType = StorageBlockType(~kBlockValueZero)
...
// orphan member: return res;
// public:
// SKIPPED C++ SYNTAX: type Difference_type = std::size_t
