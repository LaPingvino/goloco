...
func init() {
// SKIPPED CONSTRUCTOR: 	AcceptedForConsumer StationCargoStatsFlags = (1 << 0)

// // Can a producer (town building, industry) distribute cargo to
// // this station.
func init() {
// SKIPPED CONSTRUCTOR: 	AcceptedFromProducer StationCargoStatsFlags = (1 << 1)
// SKIPPED CONSTRUCTOR: 	Flag2 StationCargoStatsFlags = (1 << 2)
// SKIPPED CONSTRUCTOR: 	Flag3 StationCargoStatsFlags = (1 << 3)
...
type StationFlags int
	...
	None StationFlags = 0
// SKIPPED CONSTRUCTOR: 	TransportModeRail StationFlags = (1 << 0)
// SKIPPED CONSTRUCTOR: 	TransportModeRoad StationFlags = (1 << 1)
// SKIPPED CONSTRUCTOR: 	TransportModeAir StationFlags = (1 << 2)
// SKIPPED CONSTRUCTOR: 	TransportModeWater StationFlags = (1 << 3)
// SKIPPED CONSTRUCTOR: 	Flag_4 StationFlags = (1 << 4)
// SKIPPED CONSTRUCTOR: 	Flag_5 StationFlags = (1 << 5)
// SKIPPED CONSTRUCTOR: 	Flag_6 StationFlags = (1 << 6)
// SKIPPED CONSTRUCTOR: 	Flag_7 StationFlags = (1 << 7)
// SKIPPED CONSTRUCTOR: 	Flag_8 StationFlags = (1 << 8)
...
// //  X P P X
// //    X X
// constexpr []<World::TilePos2, 8> kPortBorderOffsets = {
// World::TilePos2{ -1, 0 },
// World::TilePos2{ -1, 1 },
// World::TilePos2{ 0, 2 },
// World::TilePos2{ 1, 2 },
...
func (s *Station) Empty() bool {
	// StationId id() const;
	// void update();
	// uint32 calcAcceptedCargo(CargoSearchState& cargoSearchState) const;
	// char* getStatusString(char* buffer);
	// bool updateCargo();
	// int32 calculateCargoRating(const StationCargoStats& cargo) const;
	// void updateLabel();
	// void invalidate();
	// void invalidateWindow();
	// void deliverCargoToStation(const uint8 cargoType, const uint8 cargoQuantity);
	// void deliverCargoToTown(uint8 cargoType, uint16 cargoQuantity);
	// void updateCargoDistribution();
	// private:
	// void updateCargoAcceptance();
	// void alertCargoAcceptanceChange(uint32 oldCargoAcc, uint32 newCargoAcc);
...
type StationPotentialCargo struct {
	Accepted uint32
	Produced uint32
	// method: PotentialCargo calcAcceptedCargoTrainStationGhost(const Station* ghostStation, const World::Pos2& location, const uint32 filter);
	// method: PotentialCargo calcAcceptedCargoAirportGhost(const Station* ghostStation, const uint8 type, const World::Pos2& location, const uint8 rotation, const uint32 filter);
	// method: PotentialCargo calcAcceptedCargoDockGhost(const Station* ghostStation, const World::Pos2& location, const uint32 filter);
	// method: PotentialCargo calcAcceptedCargoAi(const World::TilePos2 minPos, const World::TilePos2 maxPos);
	// method: void sub_491C6F(const uint8 type, const World::Pos2& pos, const uint8 rotation, const CatchmentFlags flag);
	...
	// method: void addTileToStation(const StationId stationId, const World::Pos3& pos, uint8 rotation);
	// method: void removeTileFromStation(const StationId stationId, const World::Pos3& pos, uint8 rotation);
	// method: void removeTileFromStationAndRecalcCargo(const StationId stationId, const World::Pos3& pos, uint8 rotation);
// World::StationElement* getStationElement(const World::Pos3& pos);
// std::optional<World::Pos3> getAirportMovementNodeLoc(const StationId stationId, uint8 node);
	// method: void sub_48D794(const Station& station);
	// method: void drawStationName(Gfx::DrawingContext& drawingCtx, const Station& station, uint8 zoom, bool isHovered);