...
func init() {
	None CompanyFlags = 0
// SKIPPED CONSTRUCTOR: 	Unk0 CompanyFlags = (1 << 0)
// SKIPPED CONSTRUCTOR: 	Unk1 CompanyFlags = (1 << 1)
// SKIPPED CONSTRUCTOR: 	Unk2 CompanyFlags = (1 << 2)
// SKIPPED CONSTRUCTOR: 	Sorted CompanyFlags = (1 << 3)
// SKIPPED CONSTRUCTOR: 	IncreasedPerformance CompanyFlags = (1 << 4)
// SKIPPED CONSTRUCTOR: 	DecreasedPerformance CompanyFlags = (1 << 5)
// SKIPPED CONSTRUCTOR: 	ChallengeCompleted CompanyFlags = (1 << 6)
// SKIPPED CONSTRUCTOR: 	ChallengeFailed CompanyFlags = (1 << 7)
// SKIPPED CONSTRUCTOR: 	ChallengeBeatenByOpponent CompanyFlags = (1 << 8)
// SKIPPED CONSTRUCTOR: 	Bankrupt CompanyFlags = (1 << 9)
// SKIPPED CONSTRUCTOR: 	AutopayLoan CompanyFlags = (1 << 31)
}
...
// SKIPPED CONSTRUCTOR: 	Unk0 AiPlaystyleFlags = (1 << 0)
// SKIPPED CONSTRUCTOR: 	Unk1 AiPlaystyleFlags = (1 << 1)
// SKIPPED CONSTRUCTOR: 	Unk2 AiPlaystyleFlags = (1 << 2)
// SKIPPED CONSTRUCTOR: 	Unk3 AiPlaystyleFlags = (1 << 3)
// SKIPPED CONSTRUCTOR: 	NoAir AiPlaystyleFlags = (1 << 4)
// SKIPPED CONSTRUCTOR: 	NoWater AiPlaystyleFlags = (1 << 5)
// SKIPPED CONSTRUCTOR: 	Unk6 AiPlaystyleFlags = (1 << 6)
// SKIPPED CONSTRUCTOR: 	Unk7 AiPlaystyleFlags = (1 << 7)
// SKIPPED CONSTRUCTOR: 	TownIdSet AiPlaystyleFlags = (1 << 8)
...
func IsEmpty() bool {
	...
	if isEntity() {
// SKIPPED CONSTRUCTOR: 	return EntityId(data[1])
	return EntityId.null
	// World::Pos2 getPosition() const
	return {}
	return WorldPos2{ data[0], data[1] }
	// };
	// void formatPerformanceIndex(const int16 performanceIndex, FormatArguments& args);
...
// SKIPPED CONSTRUCTOR: 	Unk25C0HashTableEntry(WorldPos3 pos, uint8 trackRoadId, uint8 direction)
	// constexpr World::Pos3 getPosition() const
func init() {
// SKIPPED CONSTRUCTOR: 	return WorldPos3(var_00, var_02 & 0xFFFE, var_04 * 4 /*World.kSmallZStep*/)

	// constexpr uint8 getTrackRoadId() const
func init() {
	return var_05 & 0x3F

	// constexpr uint8 getDirection() const
func init() {
...
func init() {
	...
	// constexpr uint16 calculateHash() const;
	// };
	var name StringId
	...
	// uint32 cargoUnitsDeliveredHistory[120]; // 0x85FC
	// int16 performanceIndexHistory[120];     // 0x87DC
	var historySize uint16
	// currency48_t companyValueHistory[120];    // 0x88CE
	var vehicleProfit currency48_t
	// uint16 transportTypeCount[6];           // 0x8BA4
	// uint8 activeEmotions[9];                // 0x8BB0 duration in days that emotion is active 0 == not active
	...
	// uint32 cargoDelivered[32];              // 0x8BCE;
	var challengeProgress uint8
	var numMonthsInTheRed uint8
	var cargoUnitsTotalDistance uint32
	// uint32 cargoUnitsDistanceHistory[120];  // 0x8C54
	var jailStatus uint16
	// CompanyId id() const;
	// bool empty() const;
	// bool isVehicleIndexUnlocked(const uint8 vehicleIndex) const;
	...
	// uint8 getHeadquarterPerformanceVariation() const;
	// bool hashTableContains(const Unk25C0HashTableEntry& entry) const;
	// bool addHashTableEntry(const Unk25C0HashTableEntry& entry);
	// private:
	// void setHeadquartersVariation(const uint8 variation);
	// void setHeadquartersVariation(const uint8 variation, const World::TilePos2& pos);
	// uint8 getNewChallengeProgress() const;
	// };
	// StringId getCorporateRatingAsStringId(CorporateRating rating);
	// constexpr CorporateRating performanceToRating(int16 performanceIndex);
	// void formatPerformanceIndex(const int16 performanceIndex, FormatArguments& args);
	// void companyEmotionEvent(CompanyId companyId, Emotion emotion);
	// void companySetObservation(CompanyId id, ObservationStatus status, World::Pos2 pos, EntityId entity, uint16 object);
...
// SKIPPED C++ SYNTAX: type AvailableTracksAndRoads = any /* sfl::static_vector<uint8, Limits::kMaxRoadObjects + Limits::kMaxTrackObjects> */ 
