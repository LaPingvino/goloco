...
// std::vector<ReceivedChunk> _requestStateChunksReceived;
	...
	// method: void processFullState([]<uint8 const> data);
	// method: void updateLocalTick();
	// method: void initStatus(string_view text);
	// method: void setStatus(string_view text);
	// method: void clearStatus();
	// method: void endStatus(string_view text);
...
// ~NetworkClient() override;
	...
	// method: uint32 getLocalTick() const;
	// method: void connect(string_view host, port_t port);
	// method: void sendChatMessage(string_view message) override;
	// method: void sendGameCommand(CompanyId company, const OpenLoco::GameCommands::registers& regs);
	// method: bool shouldProcessTick(uint32 tick) const;
	// method: void runGameCommandsForTick(uint32 tick);