...
type IUdpSocket struct {
	...
	// virtual Protocol getProtocol() const = 0;
	// virtual string getIpAddress() const = 0;
	// virtual void listen(Protocol procotol, uint16 port) = 0;
	// virtual void listen(Protocol procotol, const string& address, uint16 port) = 0;
	// virtual size_t sendData(Protocol protocol, const string& address, uint16 port, const void* buffer, size_t size) = 0;
...
// std::unique_ptr<INetworkEndpoint> resolve(Protocol protocol, const string& address, uint16 port);