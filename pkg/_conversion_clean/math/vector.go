...
// SKIPPED CONSTRUCTOR: 	return max(max(a, b), c)
...
// SKIPPED CONSTRUCTOR: 	return abs(a.X-b.X) + abs(a.Y-b.Y)
...
// SKIPPED CONSTRUCTOR: 	return abs(a.X-b.X) + abs(a.Y-b.Y) + abs(a.Z-b.Z)
...
// SKIPPED CONSTRUCTOR: 	return max(abs(a.X-b.X), abs(a.Y-b.Y))
...
// SKIPPED CONSTRUCTOR: 	return max3(abs(a.X-b.X), abs(a.Y-b.Y), abs(a.Z-b.Z))
...
// SKIPPED CONSTRUCTOR: 	return uint32(gomath.Sqrt(float64(distance)))

// Distance2D returns the Euclidean distance between two 2D points
func Distance2D(a, b Vector2) uint32 {
// SKIPPED CONSTRUCTOR: 	dx := int64(a.X - b.X)
// SKIPPED CONSTRUCTOR: 	dy := int64(a.Y - b.Y)
// SKIPPED CONSTRUCTOR: 	return Sqrt(uint32(dx*dx + dy*dy))

// Distance3D returns the Euclidean distance between two 3D points
func Distance3D(a, b Vector3) uint32 {
// SKIPPED CONSTRUCTOR: 	dz := int64(a.Z - b.Z)
// SKIPPED CONSTRUCTOR: 	return Sqrt(uint32(dx*dx + dy*dy + dz*dz))