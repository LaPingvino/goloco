...
// static string_view skipDigits(string_view s)
// while (!s.empty() && (s[0] == '.' || s[0] == ',' || isdigit(s[0])))
// s = s.substr(1);
// orphan member: return s;
// static std::pair<int32, string_view> parseNextNumber(string_view s)
// int32 num = 0;
// while (!s.empty())
// if (s[0] != '.' && s[0] != ',')
// if (!isdigit(s[0]))
...
// if (newResult / 10 != num)
// return { std::numeric_limits<int32>::max(), skipDigits(s) };
// num = newResult + (s[0] - '0');
// if (num < newResult)
// return { std::numeric_limits<int32>::max(), skipDigits(s) };
// s = s.substr(1);
