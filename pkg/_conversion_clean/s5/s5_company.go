...
// Station stations[4];  // 0x06 0x4AE Will lists stations created that vehicles will route to
	...
	NumVehicles uint8
	Var_45 uint8
// uint16 var_46[16];  // 0x4EF array of uint16 object id
// uint16 vehicles[8]; // 0x66 0x50E see also numVehicles for current size
	Var_76 currency32_t
// uint8 pad_7A[0x7C - 0x7A];
	Var_7C currency32_t
	Var_80 currency32_t
	Var_84 currency32_t
	Var_88 uint8
...
type CompanyHashTableEntry struct {
	...
	CompetitorId uint8
	OwnerEmotion uint8
// uint8 mainColours[2];            // 0x1A
// uint8 vehicleColours[10][2];     // 0x1C
	CustomVehicleColoursSet uint32
// uint32 unlockedVehicles[7];      // 0x34
...
// uint16 ownerStatus[2];                  // 0x8BC6
// uint8 pad_8BCA[0x8BCE - 0x8BCA];
...
// uint8 pad_8BCA[0x8BCE - 0x8BCA];
// uint32 cargoDelivered[32];      // 0x8BCE;
// uint8 pad_8E36[0x8DC8 - 0x8C56];
// static_assert(sizeof(CompanyType2) == 0x8DC8);
type Records struct {
// int16 speed[3];   // 0x000436 (0x0052624E)
// uint8 company[3]; // 0x00043C (0x00526254)
	Pad_43A uint8
// uint32 date[3]; // 0x000440 (0x00526258)
