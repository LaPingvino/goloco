...
type Exhaust struct {
	Base EntityBase
	// uint8 pad_24[0x26 - 0x24];
	FrameNum           uint16
	StationaryProgress int16
	// uint8 pad_2A[0x32 - 0x2A];
	WindProgress uint16
	Var_34       int16
	Var_36       int16
	// uint8 pad_38[0x49 - 0x38];
	ObjectId uint8
type ExplosionCloud struct {
	// uint8 pad_24[0x28 - 0x24];
	Frame uint16
...
// orphan member: uint16 numMovements; // 0x28 Note: this is only used by redGreen money (RCT2 legacy)
// orphan member: int32 amount;        // 0x2A - currency amount in British pounds - different currencies are probably getting recalculated
// orphan member: uint8 var_2E;        // company colour?
// uint8 pad_2F[0x44 - 0x2F];
// orphan member: int16 offsetX; // 0x44
// orphan member: uint16 wiggle; // 0x46
type Smoke struct {
	// uint8 pad_24[0x28 - 0x24];
type Splash struct {
	// uint8 pad_24[0x28 - 0x24];
type VehicleCrashParticle struct {
	// uint8 pad_24[0x02];          // 0x24
	TimeToLive uint16
	Frame      uint16
	// uint8 pad_2A[0x04];          // 0x2A
...
type IncomeStats struct {
	Day int32
	// uint8 cargoTypes[4];        // 0x57
	// uint16 cargoQtys[4];        // 0x5B
	// uint16 cargoDistances[4];   // 0x63
	// uint8 cargoAges[4];         // 0x6B
...
type Entity struct {
	// uint8 pad_24[0x80 - 0x24]; // Type specific data
