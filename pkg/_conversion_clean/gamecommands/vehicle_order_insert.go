...
type VehicleOrderInsertArgs struct {
	...
	// , orderOffset(regs.dx)
	// , rawOrder((static_cast<uint64_t>(regs.cx) << 32ULL) | static_cast<uint32>(regs.eax))
}

// MALFORMED FIELD: const VehicleOrderInsertArgsCommand any = GameCommand.vehicleOrderInsert
