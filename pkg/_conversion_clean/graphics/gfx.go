...
type G1Element struct {
	...
	// : offset(reinterpret_cast<uint8*>(static_cast<uintptr_t>(src.offset)))
	// , width(src.width)
	// , height(src.height)
	// , xOffset(src.xOffset)
	// , yOffset(src.yOffset)
...
// [[nodiscard]] constexpr uint32 recolour(uint32 image)
// return ImageIdFlags::remap | image;
// [[nodiscard]] constexpr uint32 recolour(uint32 image, ExtColour colour)
// return ImageIdFlags::remap | (enumValue(colour) << 19) | image;
// [[nodiscard]] constexpr uint32 recolour(uint32 image, Colour colour)
// func Recolour(image, Colours::toExt(colour) return
// [[nodiscard]] constexpr uint32 recolour2(uint32 image, Colour colour1, Colour colour2)
// return ImageIdFlags::remap | ImageIdFlags::remap2 | (enumValue(colour1) << 19) | (enumValue(colour2) << 24) | image;
// [[nodiscard]] constexpr uint32 recolour2(uint32 image, ColourScheme colourScheme)
// func Recolour2(image, colourScheme.primary, colourScheme.secondary) return
// [[nodiscard]] constexpr uint32 recolourTranslucent(uint32 image, ExtColour colour)
// return ImageIdFlags::translucent | (enumValue(colour) << 19) | image;
// [[nodiscard]] ImageId applyGhostToImage(uint32 imageIndex);
// [[nodiscard]] constexpr uint32 getImageIndex(uint32 imageId) { return imageId & 0x7FFFF; }
