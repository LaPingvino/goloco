...
// static string _cheatBuffer; // 0x0011364A5
// static []<Key, 64> _keyQueue;
// static uint32 _keyQueueLastWrite;
// static uint32 _keyQueueReadIndex;
// static uint32 _keyQueueWriteIndex;
// static BitSet<SDL_NUM_SCANCODES> _keyboardState;
// static bool _hasKeyboardState = false;
// static const std::pair<string, std::function<void()>> kCheats[] = {
// { "DRIVER", loc_4BECDE },
// { "SHUNT", loc_4BED04 },
// { "FREECASH", loc_4BED79 }
...
// uint32 writeIndex = _keyQueueWriteIndex;
// auto nextWriteIndex = static_cast<uint32>((writeIndex + 1) % std::size(_keyQueue));
...
// return;
// uint32 index = _keyQueueLastWrite;
// auto unsignedText = reinterpret_cast<const uint8*>(text);
// _keyQueue[index].charCode = convertUnicodeToLoco(readCodePoint(&unsignedText));
// // 0x00407028
// static std::optional<Key> getNextKey()
// uint32 readIndex = _keyQueueReadIndex;
