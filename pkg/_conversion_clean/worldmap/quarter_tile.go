...
type QuarterTile struct {
	Val uint8
	// explicit constexpr QuarterTile(uint8 tileQuarter, uint8 zQuarter)
	// : _val(tileQuarter | (zQuarter << 4))
}

// explicit constexpr QuarterTile(uint8 tileAndZQuarter)
// : _val(tileAndZQuarter)
// // Rotate both of the values amount. Returns new RValue QuarterTile
// constexpr const QuarterTile rotate(uint8 amount) const
// switch (amount)
// default:
...
// rotVal2 &= 0b00010001;
// orphan member: return QuarterTile{ static_cast<uint8>(rotVal1 | rotVal2) };
// case 2:
// auto rotVal1 = _val << 2;
// auto rotVal2 = rotVal1 >> 4;
// // Clear the bit from the tileQuarter
