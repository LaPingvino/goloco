...
// Iterator operator+(int32 amount) const
// Iterator res = *this;
// while (amount-- != 0)
// res++;
// orphan member: return res;
...
// private:
// orphan member: uint32 _beginTableOffset;
// orphan member: uint32 _currentOrderOffset;
// public:
// // currentOrderOffset is relative to beginTableOffset and is where the ring will begin and end
// OrderRingView(uint32 beginTableOffset, uint32 currentOrderOffset = 0)
...
// Order* atIndex(const uint8 index) const;
...
// std::pair<World::Pos3, string> generateOrderUiStringAndLoc(uint32 orderOffset, uint8 orderNum);
// func GenerateNumDisplayFrames(head Vehicles::VehicleHead) 
// []<const NumDisplayFrame> displayFrames();
