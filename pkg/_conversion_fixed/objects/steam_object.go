...
func (s *SteamObject) Validate() bool {
	// void load(const LoadedObjectHandle& handle, []<const std::byte> data, ObjectManager::DependentObjects* dependencies);
	// void unload();
	// constexpr bool hasFlags(SteamObjectFlags flagsToTest) const
	return (flags & flagsToTest) != SteamObjectFlags.none
	// std::pair<uint16, const ImageAndHeight*> getFramesInfo(bool isType1) const
