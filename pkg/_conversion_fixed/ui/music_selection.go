...
// window.sortMode = static_cast<uint16>(sortMode);
...
// func If(order1 oldSortMode ==) else
// newSortMode = order2;
...
// // Set status bar text
// uint16 numTracksSelected = std::ranges::count(Config::get().audio.customJukebox, true);
...
// uint16 y = 0;
// for (uint16 row = 0; row < window.rowCount; row++)
...
// const uint16 currentRow = y / kRowHeight;
...
// uint16 numTracksSelected = std::ranges::count(config.customJukebox, true);
