...
type GhostPlacedFlags int
	...
	None GhostPlacedFlags = 0
	Tree GhostPlacedFlags = 1 << 0
	Wall GhostPlacedFlags = 1 << 1

// OPENLOCO_ENABLE_ENUM_OPERATORS(GhostPlacedFlags);
// static int16 _adjustToolSize;                             // 0x0050A000
// static uint8 _adjustLandToolSize;                         // 0x009C870E
// static uint8 _clearAreaToolSize;                          // 0x009C870F
// static uint8 _adjustWaterToolSize;                        // 0x009C8710
// static uint8 _lastSelectedLand;                           // 0x00F003D2
// static uint32 _raiseLandCost;                             // 0x00F2530C
// static uint32 _lowerLandCost;                             // 0x00F25310
// static World::TreeElement* _lastPlacedTree;                 // 0x01136470
// static uint32 _lastTreeCost;                              // 0x01136484
// static World::Pos2 _terraformGhostPos;                      // 0x01136488
// static uint16 _lastTreeColourFlag;                        // 0x01136490
// static uint16 _terraformGhostTreeRotationFlag;            // 0x01136492
// static uint8 _treeRotation;                               // 0x01136496
// static Colour _treeColour;                                  // 0x01136497
// static uint8 _terraformGhostBaseZ;                        // 0x01136499
// static Common::GhostPlacedFlags _terraformGhostPlacedFlags; // 0x0113649A
// static uint8 _terraformGhostTreeElementType;              // 0x0113649B
// static uint8 _terraformGhostType;                         // 0x0113649C
// static uint8 _terraformGhostQuadrant;                     // 0x0113649D (trees)
// static uint8 _terraformGhostRotation;                     // 0x0113649D (walls)
// static uint8 _treeClusterType;                            // 0x0113649E
// static uint32 _lowerWaterCost;                            // 0x01136528
// static uint32 _raiseWaterCost;                            // 0x0113652C
...
// int32 scrollWidth = 0, scrollHeight = 0;
...
// auto treeCount = 0;
// for (uint16 i = 0; i < ObjectManager::getMaxObjects(ObjectType::tree); i++)
...
// // 0x004BD237
// func PlaceTreeGhost(placementArgs GameCommands::TreePlacementArgs) currency32_t
// removeTreeGhost();
...
// // 0x004BD1D9
// static std::optional<GameCommands::TreePlacementArgs> getTreePlacementArgsFromCursor(const int16 x, const int16 y)
...
// // 0x004BBF3B
// func ScrollMouseDown(self Window, x int16, y int16, scroll_index [[maybe_unused]] uint8)
...
// getGameState().lastTreeOption = static_cast<uint8>(rowInfo);
// updateTreeColours(self);
// int32 pan = (self.width >> 1) + self.x;
...
// break;
// // 0x004BBEF8
// func ScrollMouseOver(self Window, x int16, y int16, scroll_index [[maybe_unused]] uint8)
// auto index = getRowIndex(x, y);
// uint16 rowInfo = y;
// auto i = 0;
// for (; i < self.var_83C; i++)
...
// uint32 image = treeObj->getTreeGrowthDisplayOffset() * treeObj->numRotations;
...
// uint16 xPos = 0;
// uint16 yPos = 0;
// for (uint16 i = 0; i < self.var_83C; i++)
...
// orphan member: ClearLandArgs args{};
// args.centre = centre;
// args.pointA = pointA;
// args.pointB = pointB;
...
// // 0x004BC689
// func OnToolDown(self [[maybe_unused]] Window, widgetIndex WidgetIndex_t, id [[maybe_unused]] WidgetId, x [[maybe_unused]] int16, y [[maybe_unused]] int16)
// if (widgetIndex != Common::widx::panel)
// return;
// clearLand(Flags::apply);
// // 0x004BC682
// func ToolDrag(self [[maybe_unused]] Window, widgetIndex WidgetIndex_t, id [[maybe_unused]] WidgetId, x [[maybe_unused]] int16, y [[maybe_unused]] int16)
// if (widgetIndex != Common::widx::panel)
// return;
...
// clearLand(Flags::apply);
// // 0x004BC701
// func ToolUp(self [[maybe_unused]] Window, widgetIndex WidgetIndex_t, id [[maybe_unused]] WidgetId, x [[maybe_unused]] int16, y [[maybe_unused]] int16)
// if (widgetIndex == Common::widx::panel)
// World::mapInvalidateSelectionRect();
// World::resetMapSelectionFlag(World::MapSelectionFlags::enable);
// // 0x004BC555
...
// orphan member: FormatArguments args{};
// args.push<uint16>(_adjustToolSize);
...
// auto point = Point(xPos, yPos);
// orphan member: FormatArguments args{};
// args.push<uint32>(_raiseLandCost);
...
// auto landIndex = 0;
// for (uint16 i = 0; i < ObjectManager::getMaxObjects(ObjectType::land); i++)
// auto landObj = ObjectManager::get<LandObject>(i);
// if (landObj == nullptr)
// continue;
...
// self.invalidate();
// // 0x00468DFD
// func LowerLand(flags uint8) uint32
// orphan member: uint32 cost;
// if ((flags & 1))
// Common::sub_4A69DD();
// auto [pointA, pointB] = World::getMapSelectionArea();
...
// // 0x00468D1D
// func RaiseLand(flags uint8) uint32
// orphan member: uint32 cost;
// if ((flags & 1))
// Common::sub_4A69DD();
// auto [pointA, pointB] = World::getMapSelectionArea();
...
// auto [pointA, pointB] = World::getMapSelectionArea();
// orphan member: ChangeLandMaterialArgs args{};
// args.pointA = pointA;
// args.pointB = pointB;
...
// if (_lowerLandCost != 0)
// orphan member: FormatArguments args{};
// args.push<uint32>(_lowerLandCost);
// auto point = Point(xPos, yPos);
...
// orphan member: FormatArguments args{};
// args.push<uint32>(_raiseWaterCost);
// auto point = Point(xPos, yPos);
// tr.drawStringCentred(point, Colour::black, StringIds::increase_height_cost, args);
// yPos += 10;
...
// orphan member: FormatArguments args{};
// args.push<uint32>(_lowerWaterCost);
// auto point = Point(xPos, yPos);
// tr.drawStringCentred(point, Colour::black, StringIds::decrease_height_cost, args);
// static constexpr WindowEventList kEvents = {
// .onClose = onClose,
...
// auto wallCount = 0;
// for (uint16 i = 0; i < ObjectManager::getMaxObjects(ObjectType::wall); i++)
...
// // 0x004BD48E
// static std::optional<GameCommands::WallPlacementArgs> getWallPlacementArgsFromCursor(const int16 x, const int16 y)
// auto* self = WindowManager::find(WindowType::terraform);
// if (self == nullptr)
// return {};
// auto res = ViewportInteraction::getSurfaceLocFromUi({ x, y });
...
// if (index < 0)
// self.rowHover = rowInfo;
// getGameState().lastWallOption = static_cast<uint8>(rowInfo);
// int32 pan = (self.width >> 1) + self.x;
// Audio::playSound(Audio::SoundId::clickDown, pan);
// self.expandContentCounter = -16;
// self.invalidate();
...
// uint32 imageId = landObj->mapPixelImage + Land::ImageIds::toolbar_terraform_land;
// Widget::drawTab(self, drawingCtx, imageId, widx::tab_adjust_land);
// // Adjust Water Tab
// auto waterObj = ObjectManager::get<WaterObject>();
// uint32 imageId = waterObj->image + Water::ImageIds::kToolbarTerraformWater;
