...
// // 0x0112CE04
// static std::optional<string> _targetPath;
// // 0x0050AEA8
...
// static string getBasename(const fs::path& path);
...
// // ebx: title
// // eax: {return}
// std::optional<string> open(
// browse_type type,
// string_view savePath,
// const char* filter,
// StringId titleId)
// TextInput::cancel();
// _type = type;
...
// scrollHeight = static_cast<int32>(window.rowHeight * _files.size());
// // 0x004464F7
// func OnScrollMouseDown(self Window, x [[maybe_unused]] int16, y int16, scrollIndex [[maybe_unused]] uint8)
...
// // 0x004464B1
// func OnScrollMouseOver(self Window, x [[maybe_unused]] int16, y int16, scrollIndex [[maybe_unused]] uint8)
// if (WindowManager::getCurrentModalType() != WindowType::fileBrowserPrompt)
// return;
// auto index = y / self.rowHeight;
// if (index >= static_cast<uint16>(_files.size()))
// return;
...
// const uint16 maxWidth = self.widgets[widx::parent_button].left - folderLabelWidth - 10;
...
// uint16 folderWidth = Gfx::TextRenderer::getStringWidth(Gfx::Font::medium_bold, _displayFolderBuffer);
...
// drawTextInput(&window, drawingCtx, inputSession.buffer.c_str(), static_cast<int32>(inputSession.cursorPosition), showCaret);
...
// auto backupg1 = *g1;
// *g1 = {};
// g1->offset = (uint8*)saveInfo.image;
// g1->width = 250;
// g1->height = 200;
// drawingCtx.drawImage(x + 1, y + 1, imageId);
// *g1 = backupg1;
// y += 207;
// uint16 maxWidth = window.width - window.widgets[widx::scrollview].right;
...
// const string gbuffer = string(text, caret);
...
// auto filterExtension = string(_filter);
