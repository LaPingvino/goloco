...
// static std::vector<uint32> _inUseCompetitors;
...
// self->rowCount = static_cast<uint16>(_competitorList.size());
// self->rowHover = -1;
// self->object = nullptr;
...
// scrollHeight = static_cast<int32>(_competitorList.size()) * kRowHeight;
...
// static ObjectManager::ObjIndexPair getObjectFromSelection(const int16 y)
// const int16 rowIndex = y / kRowHeight;
// if (rowIndex < 0 || static_cast<uint16>(rowIndex) >= _competitorList.size())
...
// // 0x00435314
// func ScrollMouseDown(self Window, x [[maybe_unused]] int16, y int16, scroll_index [[maybe_unused]] uint8)
...
// if (result)
// WindowManager::close(&self);
// func If(WindowType::options _callingWindowType ==) else
// auto& config = Config::get();
// config.preferredOwnerFace = objRow.object._header;
// Config::write();
// WindowManager::close(&self);
// // 0x004352C7
// func ScrollMouseOver(self Window, x [[maybe_unused]] int16, y int16, scroll_index [[maybe_unused]] uint8)
...
// uint32 img = Gfx::recolour(competitor->images[0] + 1, Colour::black);
...
// const int16 y = index * kRowHeight;
// uint8 inlineColour = ControlCodes::Colour::black;
...
// string name = object.object._name;
