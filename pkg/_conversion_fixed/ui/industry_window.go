...
// auto industry = IndustryManager::get(IndustryId(self.number));
// int16 tileZ = World::TileManager::getHeight({ industry->x, industry->y }).landHeight;
// // Compute views.
// SavedView view = {
// industry->x,
// industry->y,
...
// args.push<uint32>(industry->receivedCargoQuantityPreviousMonth[cargoNumber]);
...
// else
// args.push(cargoObj->unitNamePlural);
// args.push<uint32>(industry->producedCargoQuantityPreviousMonth[cargoNumber]);
// args.push<uint16>(industry->producedCargoPercentTransportedPreviousMonth[cargoNumber]);
...
// const uint16 graphBottom = self.y + self.height - 7;
// int32 yTick = 0;
// for (int16 yPos = graphBottom; yPos >= self.y + 68; yPos -= 20)
// orphan member: FormatArguments args{};
// args.push(yTick);
...
// const uint8 productionTabWidx = self.currentTab + widx::tab_industry;
// const uint8 productionNum = productionTabWidx - widx::tab_production;
// for (uint8 i = industry->producedCargoMonthlyHistorySize[productionNum] - 1; i > 0; i--)
// const uint16 xPos = self.x + 41 + i;
// const uint16 yPos = self.y + 56;
...
// const uint16 yPos1 = graphBottom - history[i];
// const uint16 yPos2 = graphBottom - history[i + 1];
...
// static constexpr uint32 productionTabImageIds[] = {
...
// // Transported Tab
// static constexpr uint32 transportedTabImageIds[] = {
