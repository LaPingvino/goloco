...
type MainTabInfo struct {
	Image uint32
// []<const SubTabInfo> subTabs;
// // clang-format off
// static constexpr [] kWorldRegionSubTabs{
...
// static constexpr [] kVehicleSubTabs{
...
// static constexpr [] kTrackSubTabs{
...
// static constexpr [] kBuildingSubTabs{
...
// static uint16 _numVisibleObjectsListed;
...
// func ShouldShowSubTab(subTabs any /* []<SubTabInfo> */ , index uint8, filterLevel FilterLevel) bool
...
// continue;
// const string_view name = entry.object._name;
...
// return { static_cast<int16>(entry.index), entry.object };
// if (_tabObjectList.size() > 0)
// return { static_cast<int16>(_tabObjectList[0].index), _tabObjectList[0].object };
...
// const auto& subTabs = currentTab.subTabs;
// const bool showSecondaryTabs = !subTabs.empty() && FilterLevel(self.filterLevel) != FilterLevel::beginner;
FilterLevelStringIds = [3]StringId{
...
// int16 width = self->x + self->width - x;
// int16 height = self->y + self->height - y;
// // Clip the draw area to simplify image draw
// const auto& rt = drawingCtx.currentRenderTarget();
...
// uint8 textColour = ControlCodes::Colour::black;
...
// self.invalidate();
// // 0x004738ED
// func GetScrollSize(self [[maybe_unused]] Window, scrollIndex [[maybe_unused]] uint32, scrollWidth [[maybe_unused]] int32, scrollHeight int32) 
...
// // 0x00472B54
// static ObjectManager::ObjIndexPair getObjectFromSelection([[maybe_unused]] Window* self, int16& y)
// for (auto& entry : _tabObjectList)
// if (entry.display == Visibility::hidden)
// continue;
// y -= kRowHeight;
