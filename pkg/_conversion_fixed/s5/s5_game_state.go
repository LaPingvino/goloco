...
type Construction struct {
	// uint8 signals[8];       // 0x00015A (0x00525F72)
	// uint8 bridges[8];       // 0x000162 (0x00525F7A)
	// uint8 trainStations[8]; // 0x00016A (0x00525F82)
	// uint8 trackMods[8];     // 0x000172 (0x00525F8A)
	// uint8 var_17A[8];       // 0x00017A (0x00525F92) NOTE: not used always filled with 0xFF
	// uint8 roadStations[8];  // 0x000182 (0x00525F9A)
	// uint8 roadMods[8];      // 0x00018A (0x00525FA2)
}

// static_assert(sizeof(Construction) == 0x38);
// // We are only splitting this up to make handling the GameState2 struct easier
type GeneralState struct {
	// uint32 rng[2];                                    // 0x000000 (0x00525E18)
	// uint32 unkRng[2];                                 // 0x000008 (0x00525E20)
	Flags             uint32
	CurrentDay        uint32
	DayCounter        uint16
	CurrentYear       uint16
	...
	// uint8 playerCompanies[2];                         // 0x000024 (0x00525E3C)
	// uint16 entityListHeads[Limits::kNumEntityLists];  // 0x000026 (0x00525E3E)
	// uint16 entityListCounts[Limits::kNumEntityLists]; // 0x000034 (0x00525E4C)
	// uint8 pad_0042[0x046 - 0x042];                    // 0x000042
	// uint32 currencyMultiplicationFactor[32];          // 0x000046 (0x00525E5E)
	// uint32 unusedCurrencyMultiplicationFactor[32];    // 0x0000C6 (0x00525EDE)
	...
	// uint32 tickStartPrngState[2];                     // 0x0001B4 (0x00525FCC)
	...
	MaxLoanSize                uint16
	// uint32 multiplayerPrng[2];                        // 0x000404 (0x0052621C)
	...
	// uint16 lastMapWindowSize[2];                      // 0x000470 (0x00526288)
	LastMapWindowVar88A uint16
	LastMapWindowVar88C uint16
	Var_478             uint32
	// uint8 pad_047C[0x13B6 - 0x47C];                   // 0x00047C
	NumMessages        uint16
	ActiveMessageIndex uint16
	// Message messages[S5::Limits::kMaxMessages];         // 0x0013BA (0x005271D2)
	// uint8 pad_B95A[0xB95C - 0xB95A];                  // 0x00B95A
	Var_B95C uint8
	// uint8 pad_B95D[0xB960 - 0xB95D];                  // 0x00B95D
	Var_B960         uint8
	Pad_B961         uint8
	Var_B962         uint8
	Pad_B963         uint8
	...
	// uint8 pad_B969[0xB96C - 0xB969];                  // 0x00B969
...
type GameState struct {
	...
	// uint16 routings[S5::Limits::kMaxVehicles][S5::Limits::kMaxRoutingsPerVehicle]; // 0x442A44 (0x0096885C)
	// uint8 orders[S5::Limits::kMaxOrders];                                          // 0x461E44 (0x00987C5C)
...
type GameStateType2 struct {
	...
	// uint16 routings[S5::Limits::kMaxVehicles][S5::Limits::kMaxRoutingsPerVehicle]; // 0x440E24
	// uint8 orders[S5::Limits::kMaxOrders];                                          // 0x460224
